...
await send('Page.addScriptToEvaluateOnNewDocument', {source: `
  ...
  const fallback = async (name) => {
    if (name === 'list_microphones') return [];
...
await evaluate(`hydrateSettings(); hydrateSettings(); true`);
const newer = {...base, ui_effects:'enhanced'};
const older = {...base, ui_effects:'lite'};
await evaluate(`window.__settingsRequests[2].resolve(${JSON.stringify(newer)}); true`);
for (let i=0; i<100; i++) {
  if (await evaluate(`document.querySelector('[data-view="settings"]').dataset.settingsState === 'ready'`)) break;
  await new Promise(r => setTimeout(r, 25));
}
await evaluate(`window.__settingsRequests[1].resolve(${JSON.stringify(older)}); true`);
await new Promise(r => setTimeout(r, 100));
const race = await evaluate(`document.querySelector('[data-setting="ui_effects"]').value`);
if (race !== 'enhanced') throw new Error('stale hydration won: '+race);

await evaluate(`hydrateSettings(); true`);
await evaluate(`window.__settingsRequests[3].reject(new Error('bridge rejected')); true`);
for (let i=0; i<100; i++) {
...
console.log(JSON.stringify({delayed, ready, race, failed}));
ws.close();
"""
