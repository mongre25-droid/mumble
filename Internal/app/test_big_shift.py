...
def _migrated(model_value):
    ...
    d["model"] = model_value
    # Exercise the one non-destructive compatibility inference: this key did
    # not exist before the Big Shift, so derive it from the already-saved model.
    d.pop("english_only", None)
    d["big_shift_applied"] = False
    d["hardware_tier"] = "powerful"
    d["prompt_memory"] = True
    d["prompt_provider"] = "anthropic"
    d["anthropic_api_key"] = "retired-key"
    d["llm_provider"] = "anthropic"
    d["pro_mode"] = False
    d["history_max"] = 37
    d["mode_button_enabled"] = True
    d["island_foreign_toggle"] = False
    d["foreign_island_default_applied"] = False
    # write to a temp path and point branding at it
    fd, path = tempfile.mkstemp(suffix=".json")
    os.close(fd)
    with open(path, "w", encoding="utf-8") as fh:
        json.dump(d, fh)
    orig_path = branding.SETTINGS_PATH
    orig_dir = branding.DATA_DIR
    try:
        branding.DATA_DIR = os.path.dirname(path)
        branding.SETTINGS_PATH = path
        s = settings_mod.Settings()
        return dict(s.data)
    finally:
        branding.SETTINGS_PATH = orig_path
        branding.DATA_DIR = orig_dir
        try:
            os.remove(path)
            os.remove(path + ".bak")
        except OSError:
...
check("retired mode-key value is preserved", d.get("mode_button_enabled") is True)
...
for k, exp in (("prompt_mode_enabled", False), ("auto_format", False),
               ("english_only", True), ("island_foreign_toggle", True),
               ("deck_pinned", True)):
    check(f"default {k} = {exp!r}", settings_mod.DEFAULTS.get(k) == exp)
check("user-facing hardware tier setting removed",
      "hardware_tier" not in settings_mod.DEFAULTS)
check("inert legacy settings and credentials are preserved",
      all(k in d for k in (
          "hardware_tier", "prompt_memory", "prompt_provider",
          "anthropic_api_key")))
check("unsupported saved provider is preserved for explicit user choice",
      d.get("llm_provider") == "anthropic")
check("Foreign island visibility choice is preserved",
      d.get("island_foreign_toggle") is False)
check("Pro Mode is not enabled during upgrade", d.get("pro_mode") is False)
check("custom retention is not enlarged during upgrade", d.get("history_max") == 37)
