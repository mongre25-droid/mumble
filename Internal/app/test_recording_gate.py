...
import mumble  # noqa: E402  (heavy: pulls faster-whisper/keyboard/sounddevice)
...
print("\n== record gate — remains available during a live meeting ==")
m = _blank()
_arm(m, paused=False, model=object(), cloud=False)
m.meeting_recording = True
m.start_recording()
check("recording STARTED while meeting capture stayed active", m.recording is True)
check("meeting capture flag was preserved", m.meeting_recording is True)
...
check("start_recording does not veto an active meeting",
