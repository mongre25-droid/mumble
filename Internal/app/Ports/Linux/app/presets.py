...
def _normalise_custom(custom):
    """Return the safe, canonical subset of a custom-preset mapping."""
    out = {}
    entries = custom.items() if isinstance(custom, dict) else ()
    for raw_slot, value in entries:
    ...
    return {slot: out[slot] for slot in sorted(out)}


def _read_custom():
    """Read the store, raising on corruption so a writer fails closed."""
    try:
        with open(PRESETS_PATH, "r", encoding="utf-8") as f:
            raw = f.read()
    except FileNotFoundError:
        return {}
    # A just-created empty file is equivalent to a store that does not yet
    # exist. Other malformed JSON is not silently overwritten by a writer.
    if not raw.strip():
        return {}
    data = json.loads(raw)
    if not isinstance(data, list):
        raise ValueError("presets store must contain a JSON list")
    mapped = {}
    for entry in data:
        if isinstance(entry, dict):
            try:
                ...
                continue
            # Normalise each row before merging it so a later malformed
            # duplicate cannot erase an earlier valid entry for the slot.
            mapped.update(_normalise_custom({slot: entry}))
    return mapped


def load_custom_checked():
    """Return ``(custom, ok)`` without disguising an unreadable store."""
    try:
        return _read_custom(), True
...
def load_custom():
    """Load custom presets; atomic replacement makes this read safe.

    Mutations reload again under the external lock, preventing controller,
    Web UI, and cloud-sync processes from applying changes to stale state.
    """
    custom, _ok = load_custom_checked()
    return custom


def _atomic_write_custom(custom):
    """Durably replace the store from a unique temporary file beside it."""
    parent = os.path.dirname(os.path.abspath(PRESETS_PATH))
    os.makedirs(parent, exist_ok=True)
    rows = [dict({"slot": slot}, **value)
            for slot, value in sorted(custom.items())]
    fd = None
    tmp = None
    try:
        fd, tmp = tempfile.mkstemp(
            prefix="." + os.path.basename(PRESETS_PATH) + ".",
...
def _custom_transaction(mutate):
    """Reload, mutate, and replace once under the cross-process lock."""
    try:
        parent = os.path.dirname(os.path.abspath(PRESETS_PATH))
        os.makedirs(parent, exist_ok=True)
        with exclusive_file_lock(PRESETS_PATH) as acquired:
            if not acquired:
                print("presets save error: store is busy")
                return False
            current = _read_custom()
            working = {slot: dict(value)
                       for slot, value in current.items()}
            updated = _normalise_custom(mutate(working))
            if updated == current:
                return True
            return _atomic_write_custom(updated)
    except Exception as e:
        print("presets save error:", e)
        return False
...
def save_custom(custom):
    """Atomically replace all custom slots; return honest success status.

    Partial editors should prefer :func:`mutate_custom`; cloud pulls should use
    :func:`merge_custom`. Those APIs preserve unrelated concurrent changes.
    """
    replacement = _normalise_custom(custom)
    return _custom_transaction(lambda _current: replacement)


def mutate_custom(upserts=None, remove_slots=()):
    """Apply explicit per-slot edits/deletions in one locked transaction."""
    upserts = _normalise_custom(upserts)
    removals = set()
    for raw_slot in remove_slots or ():
        try:
            slot = int(raw_slot)
        except (TypeError, ValueError, OverflowError):
            continue
    ...
    return _custom_transaction(_mutate)


def merge_custom(upserts=None, remove_slots=()):
    """Merge a cloud delta without overwriting unrelated local slots."""
    return mutate_custom(upserts, remove_slots)


def all_presets(custom=None):
    ...
    custom = load_custom() if custom is None else _normalise_custom(custom)
