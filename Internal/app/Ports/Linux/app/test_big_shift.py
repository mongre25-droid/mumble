...
check("retired mode key force-disabled", d.get("mode_button_enabled") is False)
...
for k, exp in (("prompt_mode_enabled", False), ("auto_format", False),
               ("english_only", True), ("deck_pinned", True)):
    check(f"default {k} = {exp!r}", settings_mod.DEFAULTS.get(k) == exp)
check("retired hardware tier is not persisted",
