...
class CloudSync:
    ...
    def __init__(self, settings):
        ...
        ``settings`` must be a ``settings.Settings`` object (or any object
        with a dict-like ``.get(key)`` method).
        """
    ...
    def configure(self, url, anon_key):
        ...
        if changed_project:
            ...
            # project.  Never replay the old project's session into a new one.
            self._clear_session_file()
        self._settings.set("supabase_url", new_url)
        self._settings.set("supabase_anon_key", new_key)
        self._reset_client()
        return {"ok": True}
...
_SETTINGS_SYNC_EXCLUDE = frozenset({
    ...
    # Global shortcuts are device/platform configuration. A Windows-key binding
    # synced onto Linux can be impossible to press, and vice versa.
...
class SyncManager:
    ...
    def pull_reader_library(self):
        ...
        try:
            import reader_store as rs
            rows = self._pull_rows("user_reader_library", uid, limit=100)
            if not rows:
                self._mark_synced("reader")
    ...
    def push_presets(self):
        ...
        try:
            ...
            custom, readable = presets_mod.load_custom_checked()
            if not readable:
                raise RuntimeError("custom presets store is unreadable")
    ...
    def pull_presets(self):
        ...
        try:
            ...
            # Build a delta, then let the store reload and apply it under one
            # cross-process lock. A split load/save would erase a Web UI edit
            # made between those calls.
            upserts = {}
            remove_slots = set()
            for slot_str, data in payload.items():
                ...
                if title and instr:
                    ...
                    # An explicitly empty remote slot is a deletion. Slots
                    # absent from the payload remain untouched.
                    remove_slots.add(slot)

            if upserts or remove_slots:
                if not presets_mod.merge_custom(upserts, remove_slots):
                    raise RuntimeError(
                        "custom presets store is busy or unwritable")
