...
from ai.transport import (
    read_error_body, read_json_response_limited, read_response_limited,
)
...
def _anthropic_chat(system, user, api_key, model, timeout=60, max_tokens=None,
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout) as r:
            data = read_json_response_limited(r, timeout=timeout)
    except urllib.error.HTTPError as e:
        raise RuntimeError(
            f"API HTTP {e.code}: {read_error_body(e)[:200]}"
        ) from None
...
def _anthropic_warm(api_key, model=DEFAULT_MODEL, timeout=20):
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout) as r:
            read_response_limited(r, 64 * 1024, label="warm-up response",
                                  timeout=timeout)
        return True
    except Exception:
        return False
