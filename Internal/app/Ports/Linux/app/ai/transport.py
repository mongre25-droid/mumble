...
MAX_STREAM_RESPONSE_BYTES = 32 * 1024 * 1024
MAX_STREAM_LINE_BYTES = 1024 * 1024


def read_response_limited(response, max_bytes, label="response", timeout=None):
    """Read an HTTP body with declared-length, byte, and wall-time bounds."""
    ...
    while True:
        if deadline is not None and time.monotonic() >= deadline:
            raise RuntimeError(f"{label} exceeded the response time limit")
        remaining = limit + 1 - len(payload)
        try:
            chunk = response.read(min(65536, remaining))
        except TypeError:
            # Lightweight test doubles may expose read() without a size argument.
            chunk = response.read()
            unbounded_test_read = True
        else:
            unbounded_test_read = False
        if not chunk:
            break
        if not isinstance(chunk, (bytes, bytearray)):
            raise RuntimeError(f"{label} returned a non-byte body")
        payload.extend(chunk)
        if len(payload) > limit:
            raise RuntimeError(f"{label} exceeds the {limit}-byte safety limit")
        if unbounded_test_read:
            # A no-size read is conventionally the complete body. Calling such
            # a test double again can replay the same payload indefinitely.
            break
    return bytes(payload)


def read_error_body(error, max_bytes=MAX_ERROR_RESPONSE_BYTES):
    """Return a small decoded HTTP error detail without trusting its body size."""
    try:
        return read_response_limited(
            error, max_bytes, label="error response", timeout=2.0
        ).decode("utf-8", errors="replace")
    except Exception:
        return str(getattr(error, "reason", "HTTP error"))[:max_bytes]


def read_json_response_limited(response, timeout=None, label="JSON response"):
    raw = read_response_limited(
        response, MAX_JSON_RESPONSE_BYTES, label=label, timeout=timeout)
    return json.loads(raw.decode("utf-8"))


def iter_response_lines_limited(
    response,
    ...
    """Yield byte lines while bounding both a single line and the full stream."""
    total_limit = max(1, int(max_bytes))
    line_limit = max(1, int(max_line_bytes))
    headers = getattr(response, "headers", None)
    try:
        declared = headers.get("Content-Length") if headers is not None else None
        if declared is not None and int(declared) > total_limit:
            raise RuntimeError(
                f"{label} exceeds the {total_limit}-byte safety limit")
    except (TypeError, ValueError):
        pass
    ...
    def _validate(line):
        ...
        if not isinstance(line, (bytes, bytearray)):
            raise RuntimeError(f"{label} returned a non-byte line")
        if len(line) > line_limit:
            raise RuntimeError(
                f"{label} line exceeds the {line_limit}-byte safety limit")
        total += len(line)
    ...
    if callable(readline):
        while True:
            if deadline is not None and time.monotonic() >= deadline:
                raise RuntimeError(f"{label} exceeded the response time limit")
            try:
                line = readline(line_limit + 1)
            except TypeError:
                line = readline()
            if not line:
                break
            yield _validate(line)
        return

    # Compatibility for small test doubles that expose only iteration.
...
def _json_request(url, data=None, headers=None, timeout=60, method=None):
    ...
    def _perform_request():
        with urllib.request.urlopen(req, timeout=timeout) as r:
            return read_json_response_limited(r, timeout=timeout)
    ...
    try:
        ...
        try:
            detail = read_error_body(e, 4096)[:200]
        except Exception:
