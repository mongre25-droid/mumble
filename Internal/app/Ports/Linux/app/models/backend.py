...
def _get_free_memory_mb():
    ...
    # Linux: MemAvailable includes reclaimable page cache and is the kernel's
    # best estimate of memory that can be allocated without swapping.  The old
    # implementation only called the Windows GlobalMemoryStatusEx API, so this
    # always returned None on Linux and the advertised pressure-unload policy
        ...
        with open("/proc/meminfo", "r", encoding="ascii") as handle:
            for line in handle:
                ...
                if parts and parts[0].isdigit():
                    values[name] = int(parts[0])  # Linux reports KiB
        available_kib = values.get("MemAvailable")
        if available_kib is None:
            available_kib = sum(values.get(key, 0) for key in (
                "MemFree", "Buffers", "Cached", "SReclaimable"))
        if available_kib and available_kib > 0:
            return int(available_kib / 1024)
    except (OSError, ValueError, TypeError):
        pass

    # POSIX fallback for unusual Linux containers without /proc mounted.
    try:
    ...
    # Windows fallback retained for the shared module's cross-platform tests.
    try:
        import ctypes
