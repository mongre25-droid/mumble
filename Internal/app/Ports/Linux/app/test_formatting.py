...
print("\n== generated prompt history (no thread continuity) ==")
import os  # noqa: E402
from prompt_history import PromptHistory  # noqa: E402
_pm_path = "prompts.test.json"
try:
    pm = PromptHistory(_pm_path)
    pm.clear_history()
    pm.record("write a poem prompt", "You are a poet...")
    pm.record("make it shorter", "You are a concise poet...")
    rec = pm.all_prompts()
    check("records generated artefacts", len(rec) == 2)
    check("newest prompt first", rec[0]["prompt"].startswith("You are a concise"))
    pm2 = PromptHistory(_pm_path)
    check("Deck history persists across restarts", len(pm2.all_prompts()) == 2)
    check("empty exchange ignored", (pm.record("", ""), len(pm.all_prompts()))[1] == 2)
    stateless = ai._prompt_user_msg("new prompt", "")
    check("prompt request remains stateless",
          "write a poem prompt" not in stateless
          and "make it shorter" not in stateless
          and stateless.count("new prompt") == 1)
finally:
