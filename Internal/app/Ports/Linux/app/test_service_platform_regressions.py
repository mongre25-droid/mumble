...
def test_history_and_stats():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        before_words = combined.data["total_words"]
        real_save = combined._save
        combined._save = lambda: False
        try:
            check("stats reports a failed persistence operation",
                  combined.record(5, 1.0, "text") is False)
            check("failed stats save rolls the in-memory mutation back",
                  combined.data["total_words"] == before_words)
        finally:
            combined._save = real_save
...
def test_updater_installer_and_autostart():
    print("\n== updater and native swap safety ==")
    ...
    with tempfile.TemporaryDirectory() as td:
        nested = os.path.join(td, "release", "Internal", "app")
        os.makedirs(nested)
        entrypoint = ("mumble_mac.py" if sys.platform == "darwin" else
                      "mumble.py" if sys.platform.startswith("win") else
                      "mumble_linux.py")
        open(os.path.join(nested, entrypoint), "w").close()
        open(os.path.join(nested, "branding.py"), "w").close()
        check("full release archive runtime root is discovered",
              update._find_app_root(td) == nested)

        old_platform = update.sys.platform
        try:
            ...
            update.sys.platform = old_platform
        script = open(os.path.join(td, "apply_update.sh"),
                      encoding="utf-8").read()
        check("native swap carries the private venv",
              ".venv" in script and "requirements.txt" in script)
        check("native swap relaunches the platform entrypoint",
              "mumble_linux.py" in script)
        check("native swap uses POSIX commands",
              "mv " in script and "taskkill" not in script)
