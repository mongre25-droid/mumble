...
def _load_stdlib_platform():
    ...
    # Ask Python where its standard library lives. The previous string filter
    # rejected any perfectly valid interpreter installed under a directory
    # containing "Internal", leaving third-party ``import platform`` callers
    # without the stdlib API.
    candidates = []
    try:
        candidates.append(_pl.Path(_sysconfig.get_path("stdlib")) / "platform.py")
    except Exception:
        pass
    candidates.extend(_pl.Path(entry) / "platform.py" for entry in _sys.path)
    stdlib_path = next(
        (str(path) for path in candidates
         if path.is_file()
         and path.resolve() != _pl.Path(__file__).resolve()),
        None,
    )
    if not stdlib_path:
        return None
