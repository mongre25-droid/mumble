...
def test_layout_ordered_and_inside_pill():
    lay = ir.bar_layout({"modes": MODES, "show_foreign": False, "expanded": True})
    px0, px1 = lay["pill"]
    chips = lay["chips"]
    # When expanded, every mode gets a chip
...
def test_hit_test_maps_x_to_chip():
    lay = ir.bar_layout({"modes": MODES, "show_foreign": True, "expanded": True})
...
def test_single_mode_default():
    # Collapsed empty snap defaults to a single "Modes" placeholder chip (key=None).
    lay = ir.bar_layout({})  # no modes → defaults to Prompt only
    check("empty snap falls back to one Prompt chip",
          len(lay["chips"]) == 1 and lay["chips"][0][0] in ("prompt", None))
    # Expanded version shows the single Prompt chip.
    lay_exp = ir.bar_layout({"expanded": True})
    check("expanded empty snap shows one Prompt chip",
          len(lay_exp["chips"]) == 1 and lay_exp["chips"][0][0] == "prompt")
