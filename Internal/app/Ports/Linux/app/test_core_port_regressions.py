...
def test_ai_request_logging_never_persists_user_or_system_content():
    import ai

    source = inspect.getsource(ai.cerebras_chat_stream)
    assert "user!r" not in source
    assert "sys_preview" not in source
    assert "USER chars=" in source
    assert "endpoint_host" in source
...
def test_mode_parser_skips_malformed_and_empty_windows():
    import mode_select

    logs, modes = mode_select.process_transcript([
        None, {"type": "control"}, {"type": "button"}, {"type": "button"},
    ])
    assert modes == []
    assert [item["key"] for item in logs] == ["ControlPressTimestamp"]
    tokens = mode_select.tokens_from_windows(
        [{"word": "bad", "start": "invalid", "end": 1}], [(1, 0)], 0
    )
    assert all(token.get("value") != "bad" for token in tokens)
