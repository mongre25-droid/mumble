...
try:  # noqa: E402 - the parity module lands with the controller port
    import recording_limits  # type: ignore
except ImportError:  # keep the UI importable during partial/legacy upgrades
    class _RecordingLimits:
        DICTATION_MAX_SECONDS = 10 * 60
        DICTATION_MAX_DISPLAY = "10:00"
        MEETING_MAX_SECONDS = 4 * 60 * 60
        MEETING_MAX_DISPLAY = "4:00:00"

    recording_limits = _RecordingLimits()
...
def _clamp_design_size(work_width=None, work_height=None,
                       design_width=960, design_height=1180):
    """Fit the tall design inside a visible work area.

    Kept pure so short-screen and Retina point sizes can be regression-tested
    without importing a GUI backend.
    """
    try:
        width = int(work_width)
        height = int(work_height)
    except (TypeError, ValueError):
        return int(design_width), int(design_height)
    if width <= 0 or height <= 0:
        return int(design_width), int(design_height)
    return (
        max(640, min(int(design_width), width - 32)),
        max(480, min(int(design_height), height - 48)),
    )


def _platform_work_area():
    """Return the primary visible work area in logical pixels/points."""
...
def _webui_token_ok(req):
    """Authenticate controller/second-window requests to WEBUI_PORT.

    Refresh once on mismatch so a web window surviving a controller restart
    accepts the newly rotated token without weakening the fail-closed rule.
    """
...
class Api:
    ...
    def meeting_start_recording(self):
        ...
        # First access may include macOS TCC/device startup; avoid reporting a
        # false failure while the controller actually starts recording.
    ...
    def get_overview(self):
        ...
        return {
            ...
            "deck_pin_supported": sys.platform in ("darwin", "win32"),
            "deck_palette_supported": sys.platform in ("darwin", "win32"),
            # Hardware info for the homepage hardware-awareness suggestion
    ...
    def get_settings(self):
        keys = [
            ...
            "instant_text", "pro_mode", "llm_provider",
    ...
    def reset_window_size(self):
        ...
        try:
            win = getattr(self, "_window", None)
            if win is not None:
                area = _platform_work_area()
                size = (_clamp_design_size(*area)
                        if area else tuple(self._design_size))
    ...
    def save_vocabulary(self, terms, pairs, baseline_terms=None,
        ...
        try:
            ...
            old_terms = [str(term).strip() for term in (baseline_terms or [])
                         if str(term).strip()]
            old_pairs = {
                str(source).strip(): str(target).strip()
    ...
    def toggle_favorite(self, text, source="clipboard", when="", mode=""):
        try:
            ...
            return {"fav": False, "message": str(e)}

    def clear_transcripts(self):
        try:
            self._history().clear(); return {"ok": True}
        except Exception as e:
            return {"ok": False, "message": str(e)}
    ...
    def clear_clipboard(self):
        try:
            self._clipboard().clear(); return {"ok": True}
        except Exception as e:
            return {"ok": False, "message": str(e)}
    ...
    def clear_prompts(self):
        try:
            self._prompts().clear_history(); return {"ok": True}
        except Exception as e:
            return {"ok": False, "message": str(e)}
    ...
    def save_presets(self, rows):
        try:
            ...
            presets_mod.save_custom(custom)
    ...
    def set_onboarding_mode(self, active=True):
        ...
        try:
            if active:
                _set_window_topmost(
                    getattr(self, "_window", None),
                    getattr(self, "_title", None), False)
                _set_window_noactivate(
                    getattr(self, "_window", None),
                    getattr(self, "_title", None), False)
    ...
    def set_pinned(self, on=True):
        ...
        try:
            ...
            pass
        ok = _set_window_topmost(
            getattr(self, "_window", None),
            getattr(self, "_title", None), on)
        if not on:
    ...
    def set_deck_palette(self, on=True):
        ...
        when applied."""
        return _set_window_noactivate(
            getattr(self, "_window", None),
            getattr(self, "_title", None), bool(on))
...
def _set_window_topmost(window, title, on):
    """Use pywebview's native cross-platform topmost API when available."""
    if window is not None:
        try:
            window.on_top = bool(on)
            return bool(window.on_top) is bool(on)
        except Exception as e:
            print("native set_on_top failed:", e)
    return _set_topmost(title, on)


def _set_window_noactivate(window, title, on):
    """Toggle focus-neutral palette behavior on Windows or Cocoa.

    pywebview's Cocoa WindowHost exposes a `focus` flag through
    canBecomeKeyWindow(). Updating it on the AppKit thread lets the Deck receive
    first-mouse clicks without taking the source application's key focus.
    """
    if sys.platform.startswith("win"):
        return _set_noactivate(title, on)
    if sys.platform == "darwin" and window is not None:
        try:
            ...
            def _apply():
                instance.window.focus = not bool(on)
                if on:
            ...
            cocoa.AppHelper.callAfter(_apply)
            return True
        except Exception as e:
            print("native palette mode failed:", e)
    return False
...
def main():
    ...
    area = _platform_work_area()
    if area:
        DESIGN_W, DESIGN_H = _clamp_design_size(*area)
    if sys.platform.startswith("win"):
        try:
            import ctypes
            from ctypes import wintypes
        ...
        def _ml():
            ...
            try:
                if not onboarding and a.settings.get("deck_pinned", True):
                    _set_window_topmost(w, title, True)
            except Exception as e:
    ...
    def _ensure_main_front(*_a):
        ...
        try:
            ...
            pinned = True
        if pinned:
            _set_window_topmost(mw, title, True)
            _set_window_noactivate(mw, title, True)
            try:
                mw.show()
                ...
                pass
            if sys.platform.startswith("win"):
                _show_palette_front(title)  # Win32 no-activate restore + re-pin
            H["main_min"] = False
            return mw
        try:
            mw.show()
