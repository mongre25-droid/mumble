...
def dark_titlebar(win):
    """Make a window's title bar dark (Win 10/11 + macOS).

    On Windows, sets the DWM immersive dark mode attribute. On macOS,
    applies the NSAppearanceNameDarkAqua appearance so the title bar
    matches the dark-themed content (OLUI-005)."""
    if sys.platform.startswith("win"):
        ...
        try:
            import AppKit
            win.update_idletasks()
            # Apply the dark appearance to the underlying NSWindow so the
            # title bar and window chrome match Mumble's dark theme.
            ns_win = AppKit.NSApp().mainWindow()
            if ns_win is not None:
                ns_win.setAppearance_(
                    AppKit.NSAppearance.appearanceNamed_(
                        AppKit.NSAppearanceNameDarkAqua))
        except Exception:
            pass  # PyObjC unavailable (non-macOS dev/testing)
