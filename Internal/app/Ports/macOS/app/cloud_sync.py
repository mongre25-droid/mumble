...
class SyncManager:
    ...
    def push_reader_library(self):
        ...
        try:
            ...
            for doc in rows:
                ...
                payload = dict(doc)
                # Local paths reveal usernames/folder structure and are not
                # meaningful on another device.
    ...
    def pull_reader_library(self):
        ...
        try:
            ...
            merged = 0
            with rs._LOCK:
                # Load under the same lock used for the final save so a local
                # Reader write cannot be overwritten by a stale snapshot.
                local_rows = rs._load()
                ...
                for row in rows:
                    ...
                    if local_doc:
                        ...
                        # Reader sync uses whole-document last-write-wins until
                        # per-field tombstones/version vectors are available.
                        for k, v in payload.items():
                            local_doc[k] = v
                        merged += 1
                    else:
