...
# Keep bridge imports isolated too. Direct execution used to restore the real
# Reader path after Group G, then write CSV/XLSX/clipboard fixtures into the
# user's production library.
_bridge_temp = tempfile.TemporaryDirectory()
reader_store.PATH = os.path.join(_bridge_temp.name, "reader_library.json")
...
try:
    ...
    # Unsupported format (use a real file so this tests format dispatch rather
    # than the earlier, more accurate File not found error).
    uf = tempfile.NamedTemporaryFile(suffix=".xyz", delete=False, mode="wb")
    try:
        uf.write(b"unsupported")
        uf.close()
        result_unsup = api.reader_import_file(uf.name)
...
# HTML
html_data = b"<html><head><title>Test Page</title><script>SHOULD_NOT_LEAK</script></head><body><h1>Hello</h1><p>World</p><ul><li>Item</li></ul></body></html>"
...
check("VAL-READER-064c: HTML strips script/style (no script text leaked)",
      "SHOULD_NOT_LEAK" not in doc_html.plain_text())
...
try:
    ...
    print("  [FAIL] EPUB tests failed:", e)
    check("VAL-READER-065: EPUB parser does not raise", False)
