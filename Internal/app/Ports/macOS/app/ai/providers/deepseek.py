"""DeepSeek AI provider — deepseek-v4-flash via OpenAI-compatible API.

DeepSeek is a benchmarking-only provider; it is hidden from the Settings UI
...
class DeepSeekProvider(BaseProvider):
    ...
    def headers(self):
        """Return Bearer-token headers for DeepSeek requests."""
    ...
    def chat_stream(self, messages, **kwargs):
        ...
        """
        yield from _deepseek_chat_stream(
            messages,
            api_key=self._api_key,
            model=kwargs.get("model", self._model),
            temperature=kwargs.get("temperature", 0.2),
    ...
    def key_ok(api_key, timeout=10):
        ...
        """
        return _key_ok(api_key, timeout=timeout, models_url=DEEPSEEK_MODELS_URL)
    ...
    def fetch_models(api_key, timeout=12):
        ...
        """
        return _fetch_models(api_key, DEEPSEEK_MODELS_URL, timeout=timeout)
...
def _deepseek_chat(
    ...
    if finish == "length":
        print(
            f"[deepseek_chat] WARNING: response truncated "
...
def _key_ok(api_key, timeout=10, models_url=DEEPSEEK_MODELS_URL):
