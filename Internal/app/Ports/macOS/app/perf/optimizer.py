...
Thread counts are tuned for interactive latency rather than bulk throughput:
  • STT: physical-core-scale — avoids SMT/Hyper-Thread contention
...
def physical_cpu_count(logical_count=None):
    """Best-effort physical-core count for latency-sensitive inference."""
    logical = max(1, int(logical_count or (os.cpu_count() or 1)))
    physical = None
    try:
        import psutil
        ...
        physical = None
    if not physical and sys.platform == "win32" and logical >= 8:
        # Conservative fallback for the common two-way SMT layout. It is safer
        # for interactive latency to under-use a hybrid CPU than to saturate all
        # logical processors and contend with the UI/audio threads.
        physical = (logical + 1) // 2
    if not physical:
        physical = logical
    return max(1, min(logical, int(physical)))


def recommended_stt_threads(logical_count=None, physical_count=None):
    """Return a physical-core-scale budget for one interactive STT decode."""
    logical = max(1, int(logical_count or (os.cpu_count() or 1)))
    physical = (physical_cpu_count(logical)
                if physical_count is None else int(physical_count))
    return max(1, min(logical, max(STT_THREAD_FLOOR, physical)))
