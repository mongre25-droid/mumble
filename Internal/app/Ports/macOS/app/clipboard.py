...
def _macos_grab_image():
    """Grab the current image from the macOS pasteboard (NSPasteboard), returning
    a PIL Image or None. Uses the public TIFF pasteboard type — macOS stores
    images as TIFF on the general pasteboard, which PIL can decode natively."""
    import sys
    if sys.platform != "darwin":
        return None
    try:
        ...
        return Image.open(BytesIO(tiff_data))
    except Exception:
        return None


def _grab_image():
    """Cross-platform image clipboard grab. Tries ImageGrab (Windows) first,
    then NSPasteboard (macOS). Returns a PIL Image or None."""
    try:
    ...
    if isinstance(grabbed, Image.Image):
        return grabbed
    # macOS fallback — ImageGrab.grabclipboard() only works on Windows
    img = _macos_grab_image()
    if img is not None:
        return img
    # Windows can also return a list of file paths (see _loop below)
    return grabbed
...
class Clipboard:
    def __init__(self, path, maxlen=25, img_dir=None):
        ...
        # _grab_image() is a real Win32 bitmap grab — the most expensive
    ...
    def _loop(self, stop_event):
        while self._running and not stop_event.wait(0.7):
            try:
                ...
                # GDI `_grab_image()` — a real Win32 clipboard-open +
                    ...
                    if cur and cur != self._last_text:
                        ...
                        if cur.strip():
                            if self._add_text(cur):
                                self._fire_change()
                # Image-clipboard poll. _grab_image() is a real
    ...
    def _evicted_image_path_if_full(self):
        ...
        Deletion happens only after the updated metadata is durable.
        """
        ...
        with self._lock, exclusive_file_lock(self.path) as acquired:
            ...
            for e in list(self.items):
                if e.get("type") == "image" and e.get("hash") == h:
                    # Reuse the same full-hash PNG and move metadata to newest.
                    self.items.remove(e)
                    break
            evicted_path = self._evicted_image_path_if_full()
