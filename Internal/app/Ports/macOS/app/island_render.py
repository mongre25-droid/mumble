...
def _measure_zones(snap):
    ...
    right_w = 0.0
    if timer:
        right_w = SEP_PREFIX * s + _text_w(_MEASURE, timer, _mono_f())
    elif hint:
...
def render(snap):
    ...
    pw = pill_width(snap)
    bg, mask, geom = _build_bg(pw, rim_c)
    px0, py0, px1, py1, rad, cy = geom
    W, H = WIN_W * s, WIN_H * s
    img = bg.copy()
    ...
    # ---- right zone: separator + timer / hint, then optional offline ring ----
    rxx = rx
    if timer:
        d.line([(rxx + 5 * s, cy - 5 * s), (rxx + 5 * s, cy + 5 * s)],
               fill=_rgba(_RIM, 200), width=max(1, s))
        d.text((rxx + 13 * s, base_y), timer, font=mono_f,
               fill=_rgba(C.gold), anchor="ls")   # gold timer, baseline-aligned
        rxx += SEP_PREFIX * s + _text_w(d, timer, mono_f)
    elif hint:
        d.line([(rxx + 5 * s, cy - 5 * s), (rxx + 5 * s, cy + 5 * s)],
               fill=_rgba(_RIM, 200), width=max(1, s))
...
def bar_layout(snap):
    ...
    a click always lands on the chip you see. Returns a dict:
        {'pill': (x0, x1),
         'chips': [(mode_key, x0, x1), ...],
         'sep_x': float,
         'deck': (x0, x1),
         'foreign': (x0, x1) | None}
    `snap` carries: modes [(key,label)...], show_foreign (bool)."""
    s = SCALE
    bf = _bar_font()
    ...
    return {"pill": (px0, px0 + pill_w), "chips": chips,
            "sep_x": sep_x, "deck": deck, "foreign": foreign}
...
def _demo():
    ...
    snaps = {
        ...
        "saved": dict(state="done", label="Saved", hint="List",
                      colors=[MODE_COLORS["list"]], dot=MODE_COLORS["list"],
                      rim=rim(MODE_COLORS["list"]), label_color=MODE_COLORS["list"]),
        "done_offline": dict(state="done", label="Pasted!", hint="Text",
                             colors=[C.gold], dot=C.gold, offline=True,
                             rim=rim(C.gold), label_color=C.gold),
        "hint": dict(state="hint", label="Ctrl + Alt + H", dot=C.gold,
    ...
    MODES = [("prompt", "Prompt"), ("email", "Email"),
             ("list", "List"), ("reply", "Reply")]   # the real cloud lanes
    bar_cases = [
        ...
        ("bar_reply_foreign", dict(modes=MODES, active="reply",
