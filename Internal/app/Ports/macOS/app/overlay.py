...
readable state label (HTML-island parity, v3.0.0). States:
    ...
    hint         -> gold-pulsing "Ctrl + Alt + H" text INLINE in the pill — your
...
# The secondary "Ctrl + Alt + H" reminder island — half height, popped above.
...
class _GlassPill:
    ...
    BOTTOM_GAP = 14  # px from the window's bottom edge to the monitor bottom
...
class _WidgetBar:
    """THE BIG SHIFT — the island's companion control bar: a glass pill rendered
    through island_render's pipeline, sharing the island's liquid-glass design
    language (same gradient body, gold bloom, breathing rim). Pinned directly
    above the island with ZERO gap so they form one continuous control surface.
    ...
    def __init__(self, root, on_mode=None, on_deck=None, on_foreign=None,
        ...
        # mode, on_deck() opens the Deck, on_foreign() toggles Foreign; get_state()
        # returns the live snapshot {modes, active, foreign_on, show_foreign}.
    ...
    def place_above(self, ix, iy, iw):
        """Centre the bar horizontally over the island window and sit it so the
        bar pill's bottom edge touches the island pill's top edge (GAP=0).
        The island pill is centred in its 64px-tall canvas: its top is at
        (WIN_H - PILL_H)/2 = 15 px from the window top. The bar pill sits at the
        bottom of its canvas. Positioning the bar window 15 - BAR_WIN_H px above
        the island window makes the two glass bodies touch."""
        if not self.ok:
            return
...
class Island:
    ...
    def __init__(self, root, style=None):
        ...
        self.deck_open = False    # True while the History window (Ctrl+Alt+H) is up
        ...
        # ITEM 3/4 — the companion bar is the island's MODE DECK: a row of
    ...
    def hint(self, text):
        self.hint_text = text or "Ctrl + Alt + H"
    ...
    def close_deck(self):
        """Dismiss the History window if it's open (the Ctrl+Alt+H hotkey, owner
    ...
    def _build_snapshot(self):
        ...
        if state == "listening":
            ...
            # Ctrl+Alt+H", never imply it was already pasted).
            hint = getattr(self, "done_label", "") if pasted else "Ctrl + Alt + H"
    ...
    def _render(self):
        ...
        if state == "building" and getattr(self, "build_label", ""):
            ...
            dl = getattr(self, "done_label", "") if pasted else "Ctrl + Alt + H"
    ...
    def _paint_frame(self):
        ...
        if active:
            ...
            if self.glass.ok:
                ...
                if drawn:
                    # The Big Shift companion bar tracks the glass island.
                    # Render the bar each frame so the gold sheen slides and
                    # the Prompt ring breathes.
                    try:
                        self.widget.show_above(x, y, self.glass.W)
    ...
    def show_deck(self, items, on_complete=None, on_fav=None):
        """The History deck — Mumble's mini dashboard (default Ctrl+Alt+H). ONE surface unifying:
