...
readable state label (HTML-island parity, v3.0.0). States:
    ...
    hint         -> gold-pulsing "Ctrl + Option + H" text INLINE in the pill — your
...
TODO (CQ-005, OLUI-009): island_render.py (979 lines) is loaded but its full
premium rendering path is only active when the PyObjC glass pill (_GlassPillMac)
is available. On non-macOS dev machines or when PyObjC is missing, the Tk canvas
fallback renders instead — which visually matches but lacks the per-pixel-alpha
polish. This is a known, documented gap; no user-facing impact on actual macOS.

TODO (CQ-005, OLUI-011): The overlay checks accessibility permissions via
AXIsProcessTrusted() but the result is only used for hotkey failure notification
in the controller. The island itself does not yet surface a dedicated
"accessibility" warning state. A future refinement should add an island state
for this so the user gets a visual cue (not just a notification) when hotkeys
...
# Platform detection: ctypes Win32 APIs (LayeredWindow, MonitorFromWindow) are
# Windows-only. The macOS island uses Tk's native -transparent attribute instead.
_HAVE_CTYPES = sys.platform.startswith("win")
...
_TRANSPARENT = "#ff00ff"
# macOS Aqua Tk has no Windows-style -transparentcolor colour-key. Instead, a
# Toplevel/Canvas whose background is the system "systemTransparent" colour, with
# the window's -transparent attribute set, renders truly clear so only the drawn
# pill shows (the colour-key path raised TclError and left a solid magenta box).
_CLEAR = "systemTransparent"
_PILL_BG = "#0C0B09"
_PILL_RIM = "#3A3320"
_PILL_HI = "#5A5240"  # warm hairline highlight (top inner edge of the pill)
_ARMED_RING = "#F3EEE1"  # near-white ring for the current explicit mode
...
# Completion-flash duration (owner v9: "transient states should auto-dismiss in
# ~1.5–2.5s, target ~2s"). 30 ticks ≈ 1.35s at the 45ms on-screen rate — long
# enough to read "Pasted!/Saved · Mode", short enough that nothing lingers.
# Every completion/feedback flash (paste, copy, convert done) uses this ONE value.
...
# Synced from the Windows canonical overlay.py for cross-platform visual parity
# (VAL-MAC-009, VAL-MAC-034).
...
# The secondary "Ctrl + Option + H" reminder island — half height, popped above.
MINI_W, MINI_H = 108, 15
MINI_PILL_W = 102
MINI_GAP = 6
HINT_FRAMES = 90         # ~4s paste hint (owner v9: trimmed from ~7s)
HINT_DELAY = 9
# Legacy clarification chips fade like every other transient chip. If a mode is
# active, the old correction chip stays visible until that explicit choice clears.
...
# One source of truth for states that keep the Mac island visible. Voice Search
# previously entered a controller state that every visibility gate treated as
# idle, so the island disappeared precisely when feedback was most important.
...
class _MacWidgetBar:
    """THE BIG SHIFT — the island's companion control bar for macOS.

    A Tk-based mode deck that renders mode chips, a Foreign toggle, and a Deck
    button directly above the island. Uses island_render.render_bar() for the
    premium glass-bar image when Pillow is available, falling back to drawn
    canvas chips otherwise. Synced from the Windows _WidgetBar for visual parity
    (VAL-MAC-034).

    The bar runs in its own transparent Tk window so clicks on chips land without
    stealing typing focus. Fully fail-safe: if setup fails, `ok` is False and
    every method is a no-op.
    """
    def __init__(self, root, on_mode=None, on_deck=None, on_foreign=None,
                 on_expand=None, get_state=None):
        self.ok = False
        self.win = None
        self.visible = False
        self.canvas = None
        ...
        try:
            ...
            try:
                ...
                self.win.config(bg=_CLEAR)
            except tk.TclError:
                self.win.config(bg=_TRANSPARENT)
            self.canvas = tk.Canvas(self.win, width=self._W, height=self._H,
                                    bg=_CLEAR, highlightthickness=0, bd=0)
            self.canvas.pack()
            ...
            print("macOS widget bar unavailable (island continues):", e)
            self.ok = False

    def _on_click(self, event):
        """Route a mouse-down to the chip / Deck / Foreign zone it landed in."""
        if not self.ok:
            return
        try:
            ...
            return
        x = event.x
        expanded = bool(layout.get("expanded"))
        cz = layout.get("caret")
        if cz and cz[0] <= x < cz[1]:
            self._fire(self.on_expand)
            return
    ...
    def place_above(self, ix, iy, iw):
        """Centre the bar horizontally over the island window so the bar pill's
        bottom edge touches the island pill's top edge (GAP=0)."""
        if not self.ok:
            return
        island_pill_top = iy + 15
        x = int(ix + (iw - self._W) / 2)
        y = int(island_pill_top - self._H)
        if (x, y) != self._geo:
            self._geo = (x, y)
            try:
    ...
    def render_and_push(self, frame):
        """Render a fresh mode-deck image and push it to the canvas."""
        if not self.ok or not self.canvas:
            return False
        try:
    ...
    def _push(self, pil_img):
        ...
        try:
            ...
            # Convert PIL Image → tk.PhotoImage for canvas display.
            # PIL Image is RGBA; we paste onto a Tk-compatible RGB image.
            w, h = pil_img.size
            self._photo = _tk.PhotoImage(width=w, height=h)
            # Build the Tk image one pixel row at a time (PIL→Tk conversion).
            rgb_data = pil_img.convert("RGB").tobytes("raw", "RGB")
            # tk.PhotoImage.put() expects a format string; we pass raw RGB triplets.
            # For simplicity, create a PPM-format string and load it inline.
            self.canvas.delete("all")
            # Build a simple PPM header + data inline
            ppm_header = f"P6\n{w} {h}\n255\n".encode("ascii")
            ppm_data = ppm_header + rgb_data
            self._photo = _tk.PhotoImage(data=ppm_data)
    ...
    def show_above(self, ix, iy, iw):
        if not self.ok:
            return
        self.place_above(ix, iy, iw)
        if not self.visible:
            try:
                self.win.deiconify()
                self.win.attributes("-topmost", True)
...
class _GlassPillMac:
    """macOS-native per-pixel-alpha glass pill via PyObjC (NSWindow + CALayer).

    Renders island_render output (PIL Image → CGImage → CALayer) into a borderless
    NSWindow with per-pixel alpha, matching the Windows _GlassPill's visual quality.
    Falls back gracefully if PyObjC is unavailable — `ok` will be False and every
    method is a no-op, so the reliable Tk canvas pill renders instead.

    Synced from the Windows _GlassPill for cross-platform visual parity (VAL-MAC-009).
    """
    def __init__(self, root):
        ...
        self._geo = None
        self._tk_geo = None
        self._frame_geo = None
        self._last_place_check = 0.0
        self._root = root  # Tk root for coordinate fallback
        # DEEP-012: Pre-allocate a reusable buffer for CGImage pixel data to
        # reduce per-frame allocation pressure (~3.8 MB/sec at 22 FPS).
        self._image_buffer = None

        if not _HAVE_GLASS:
            return

        # Try to import PyObjC modules. On non-macOS (dev/testing), this will
        # fail gracefully and the Tk canvas fallback will handle rendering.
        try:
            import AppKit
            import Quartz
            ...
            print("PyObjC unavailable — glass pill will use Tk canvas fallback")
            return

        try:
            # Create a borderless, transparent NSWindow.
            frame = NSMakeRect(0, 0, 380, 60)
            self.win = AppKit.NSWindow.alloc().initWithContentRect_styleMask_backing_defer_(
                frame, AppKit.NSBorderlessWindowMask,
            ...
            # Set up a CALayer-backed view for per-pixel-alpha rendering.
            self._view = AppKit.NSView.alloc().initWithFrame_(frame)
            self._view.setWantsLayer_(True)
            self._layer = Quartz.CALayer.layer()
            self._layer.setContentsGravity_("resizeAspect")
            self._view.setLayer_(self._layer)
            self.win.setContentView_(self._view)

            self.ok = True
        except Exception as e:
            print("PyObjC glass pill init failed — using Tk canvas fallback:", e)
            self.ok = False

    def place(self, force=False):
        """Position the NSWindow centred near the bottom of the active monitor.
        Returns its Cocoa origin; `self._tk_geo` holds the equivalent Tk origin
        for the companion bar."""
        if not self.ok:
            return 0, 0
        now = time.monotonic()
        if (not force and self._geo is not None
                and now - self._last_place_check < 0.5):
            return self._geo
        self._last_place_check = now
        geometry = _mac_monitor_geometry()
        if geometry:
            tk_rect, cocoa_rect = geometry
            cleft, cbottom, cright, _ctop = cocoa_rect
            tleft, _ttop, tright, tbottom = tk_rect
            x = int(cleft + (cright - cleft - 380) / 2)
            y = int(cbottom + BOTTOM_MARGIN)
            tx = int(tleft + (tright - tleft - 380) / 2)
            ty = int(tbottom - H - BOTTOM_MARGIN)
            self._tk_geo = (tx, ty)
        else:
            # Fallback: use the primary screen via AppKit.
            try:
                screen = self._AppKit.NSScreen.mainScreen()
                frame = screen.visibleFrame()
                x = int(frame.origin.x + (frame.size.width - 380) / 2)
                y = int(frame.origin.y + BOTTOM_MARGIN)
                self._tk_geo = (
                    int((self._root.winfo_screenwidth() - 380) / 2),
                    int(self._root.winfo_screenheight() - H - BOTTOM_MARGIN),
                )
            except Exception:
                x, y = 100, 100
                self._tk_geo = (100, 100)
        self._geo = (x, y)
        try:
    ...
    def push(self, pil_img, x, y):
        """Push a PIL Image to the CALayer for display. Returns True on success."""
        if not self.ok:
            return False
        try:
            w, h = pil_img.size
            # Convert PIL RGBA → CGImage.  DEEP-012: reuse a pre-allocated
            # bytearray buffer when the image size hasn't changed so we avoid
            # re-allocating ~4KB every frame (~88 MB/sec at 22 FPS).
            data = pil_img.tobytes("raw", "RGBA")
            buf_size = len(data)
            if self._image_buffer is None or len(self._image_buffer) < buf_size:
                self._image_buffer = bytearray(buf_size)
            # Copy into the reusable buffer so CGDataProvider can reference
            # memory that outlives the temporary `data` bytes.
            self._image_buffer[:buf_size] = data
            Quartz = self._Quartz
            color_space = Quartz.CGColorSpaceCreateDeviceRGB()
            provider = Quartz.CGDataProviderCreateWithData(
                None, self._image_buffer, buf_size, None
            )
            ...
            frame_geo = (int(x), int(y), int(w), int(h))
            if frame_geo != self._frame_geo:
                frame = self._NSMakeRect(float(x), float(y), float(w), float(h))
                self.win.setFrame_display_(frame, True)
                self._frame_geo = frame_geo
            return True
        except Exception as e:
            print("glass push failed — falling back to canvas:", e)
            self.ok = False
            return False


def _cocoa_rect_to_tk(x, y, width, height, primary_top):
    """Convert a Cocoa bottom-left screen rect to Tk/Quartz top-left coords."""
    left = int(x)
    top = int(primary_top - (y + height))
    return left, top, left + int(width), top + int(height)


def _mac_monitor_geometry():
    """Return `(tk_rect, cocoa_rect)` for the frontmost app's visible screen.

    Quartz window bounds use a top-left origin while NSScreen frames use a
    bottom-left origin. Comparing either without conversion selected the wrong
    monitor in vertical arrangements and fed Tk coordinates to NSWindow.
    """
    try:
        import AppKit
        import Quartz
        ...
        primary_top = (primary_frame.origin.y + primary_frame.size.height)
        chosen = screens[0]
        ...
        if front_pid is not None:
            ...
            for info in windows:
                if info.get(Quartz.kCGWindowOwnerPID) != front_pid:
                    continue
                ...
                for screen in screens:
                    frame = screen.frame()
                    rect = _cocoa_rect_to_tk(
                        frame.origin.x, frame.origin.y,
                        frame.size.width, frame.size.height, primary_top)
        ...
        x, y = float(visible.origin.x), float(visible.origin.y)
        width, height = float(visible.size.width), float(visible.size.height)
        cocoa = (int(x), int(y), int(x + width), int(y + height))
        tk_rect = _cocoa_rect_to_tk(x, y, width, height, primary_top)
        return tk_rect, cocoa
    except Exception:
        return None
...
def _monitor_rect():
    ...
    On Windows this uses ctypes Win32 MonitorFromWindow. On macOS, uses PyObjC
    NSScreen + NSWorkspace to detect the frontmost app's screen. Falls back to
    the primary display if PyObjC is unavailable (VAL-MAC-011)."""
    if not _HAVE_CTYPES:
        geometry = _mac_monitor_geometry()
    ...
    if _HAVE_CTYPES:
        ...
        try:
            ...
            pass
    # macOS: try to detect the frontmost app's screen via PyObjC.
    else:
        try:
            import AppKit
            import Quartz
            # Get the frontmost application's process ID via NSWorkspace.
            ws = AppKit.NSWorkspace.sharedWorkspace()
            front_app = ws.frontmostApplication()
            front_pid = front_app.processIdentifier()
            # Find the screen that contains the frontmost window.
            screens = AppKit.NSScreen.screens()
            if screens:
                # Default to the screen with the menu bar (primary).
                primary = screens[0]
                frame = primary.frame()
                # For multi-monitor: try to find the screen containing the
                # frontmost app's main window.
                try:
                    app_ref = AppKit.NSRunningApplication.runningApplicationWithProcessIdentifier_(front_pid)
                    if app_ref:
                        # Get the frontmost window's frame via Quartz.
                        ...
                        for win_info in win_list:
                            if win_info.get(Quartz.kCGWindowOwnerPID) == front_pid:
                    ...
                    pass  # fall back to primary screen
                frame = primary.frame()
                # Convert macOS coords (origin bottom-left) to Tk coords (origin top-left).
                # Get total desktop height from all screens.
                max_y = max(s.frame().origin.y + s.frame().size.height for s in screens)
                x = int(frame.origin.x)
                y = int(max_y - frame.origin.y - frame.size.height)
            ...
            pass  # any PyObjC error — fall back gracefully
    # macOS / non-Windows without PyObjC: return None → _place() falls back
    # to screen geometry.
    return None


# Win32 layered-window globals (Windows-only — unused on macOS; kept for
# module-level compatibility with the Windows overlay.py). The _GlassPill
# import guard at the top prevents any actual Win32 code from running here.
_W32 = None
...
class Island:
    ...
    def __init__(self, root, style=None):
        ...
        self.frame = 0
        self.visible = False
        self.armed = False  # explicit mode active → show its armed ring
        ...
        self.deck_open = False    # True while the History window (Ctrl+Option+H) is up
        ...
        # The premium renderer: a macOS-native per-pixel-alpha NSWindow painted by
        # island_render (Pillow → CGImage → CALayer). If PyObjC is unavailable
        # (e.g., running on Windows for dev/testing), falls back to the Tk canvas
        # pill. The glass pill is the PRIMARY rendering path; the canvas is the
        # reliable fallback. Synced from the Windows _GlassPill (VAL-MAC-009).
        self.glass = _GlassPillMac(self.root)

        # THE BIG SHIFT — the companion mode-deck bar (mode chips + Deck + Foreign)
        # pinned just above the island. Uses island_render.render_bar() for the
        # premium glass-bar image displayed on a Tk canvas. Synced from the Windows
        # _WidgetBar for cross-platform visual parity (VAL-MAC-034).
        self.widget = _MacWidgetBar(
            self.root,
        ...
        # THE BIG SHIFT — the companion control bar (Prompt toggle + Deck button)
        # pinned just above the island. Controller wires the real callbacks via
        # set_widget_callbacks(); the bar reads `self.armed` to light the Prompt
        # pill (armed now mirrors the sticky Prompt toggle). Prompt status is
        # shown directly on the island via the purple ring and "✦" indicator.
        self.on_prompt_toggle = None
        self.on_deck = None
        self.on_foreign = None
        self.on_mode = None       # canonical Windows callback name for mode selector
        # ITEM 3/4 — the companion bar is the island's MODE DECK. `armed_color` lets
        # the island ring echo the active mode's colour (cohesion with the bar),
        # defaulting to Prompt purple. Synced from the Windows canonical overlay.py.
        ...
        self._was_active = False     # appear fade-in trigger
        self._appear_left = 0        # appear fade-in countdown
        ...
        self.canvas = tk.Canvas(self.win, width=W, height=H, bg=_CLEAR,
    ...
    def _make_window(self):
        ...
        try:
            ...
            # macOS: a genuinely transparent window. The Windows -transparentcolor
            # colour-key attribute is unsupported on Aqua and raises TclError, which
            # used to leave the pill sitting on a solid magenta (#ff00ff) box.
            win.attributes("-transparent", True)
            win.config(bg=_CLEAR)
        except tk.TclError:
    ...
    def hint(self, text):
        self.hint_text = text or "Ctrl + Option + H"
    ...
    def set_armed(self, on):
        ...
        held mode key is retired). When on, the island rings in Prompt purple —
        so you can SEE Prompt mode is on. Also updates armed_color for snapshot
        parity with Windows (VAL-MAC-009)."""
        self.armed = bool(on)
        if on:
            self.armed_color = MODE_COLORS.get("prompt", C.gold)

    def suggest(self, text):
        """Show the legacy dismissible 'was this a <mode>?' chip (violet)."""
    ...
    def set_widget_callbacks(self, on_prompt=None, on_deck=None, on_foreign=None, on_mode=None):
        """Wire the bar's buttons to the controller (toggle Prompt / open the Deck /
        toggle Foreign / select a mode). on_mode is the primary mode selector
        (canonical Windows name); on_prompt is the legacy alias."""
        self.on_prompt_toggle = on_prompt
        self.on_deck = on_deck
        self.on_foreign = on_foreign
        # Store on_mode as the canonical Windows callback name.
    ...
    def set_bar_state(self, modes=None, active="\x00", foreign_on=None,
        ...
        Synced from the Windows canonical overlay.py (VAL-MAC-034)."""
        st = self.bar_state
    ...
    def _widget_prompt(self):
        if callable(self.on_prompt_toggle):
            self.on_prompt_toggle()
    ...
    def _widget_mode(self, key):
        """Select a mode from the companion bar. Collapses the expanded selector
        back to a single chip (refinement pass §1)."""
        self.bar_state["expanded"] = False
        self._refresh_bar()
        # Keep the legacy prompt callback compatible if an older host wires it.
        if callable(self.on_prompt_toggle):
            self.on_prompt_toggle()
        # Also call on_mode if wired (canonical Windows name).
        on_mode = getattr(self, "on_mode", None)
        if on_mode is not None and callable(on_mode):
            on_mode(key)

    def _widget_expand(self):
        """Toggle the compact mode selector open/closed. Purely local UI state."""
        self.bar_state["expanded"] = not self.bar_state.get("expanded", False)
        self._refresh_bar()

    def _refresh_bar(self):
        """Refresh the companion bar display after a state change."""
        try:
            if getattr(self, "widget", None):
                self.widget.render_and_push(self.frame)
        except Exception:
            pass

    # ---- internals (main thread) ----
    def _click_through(self, win, canvas):
        """Make the non-interactive island pass pointer events to the app below."""
        if not _HAVE_CTYPES:
            if sys.platform == "darwin":
                try:
                    import objc
                    win.update_idletasks()
                    view = objc.objc_object(c_void_p=int(win.winfo_id()))
                    native = view.window() if hasattr(view, "window") else None
                    if native is not None:
                        native.setIgnoresMouseEvents_(True)
                        self._fallback_native_window = native
                except Exception as e:
                    print("macOS fallback click-through unavailable:", e)
            return
        try:
            import ctypes
    ...
    def close_deck(self):
        """Dismiss the History window if it's open (the Ctrl+Option+H hotkey, owner
    ...
    def _show(self):
        ...
        try:
            ...
            # runs monitor detection; skipping it when the foreground
    ...
    def _armed_ring(self, c, x0, pill_w, w, h):
        """A thin ring around the pill while an explicit mode is active."""
    ...
    def _build_snapshot(self):
        ...
        if state == "listening":
            ...
            # Ctrl+Option+H", never imply it was already pasted).
            hint = getattr(self, "done_label", "") if pasted else "Ctrl + Option + H"
        ...
        # so a stray wake() never double-advances it. Synced from Windows canonical.
        ...
        return {
            ...
            # the mode-deck above), defaulting to Prompt purple. Synced from Windows
            # canonical overlay.py (VAL-MAC-009, VAL-MAC-034).
    ...
    def _anim_span(self):
        ...
        return {"listening": 51, "search": 51, "transcribing": 20,
                "building": 32, "done": 51}.get(self.state, 0)
    ...
    def _render(self):
        ...
        if state == "building" and getattr(self, "build_label", ""):
            ...
            dl = getattr(self, "done_label", "") if pasted else "Ctrl + Option + H"
        ...
        if self.armed and state in ("listening", "transcribing", "building"):
        ...
        if is_hint:
            ...
            base = {"listening": C.gold, "search": "#5AA9E6",
        ...
        if state in ("listening", "search") and self.enhanced:
        ...
        if state in ("listening", "search"):
            ...
            for i in range(n):
                ...
                xi = x + i * gap
                wave_color = "#5AA9E6" if state == "search" else C.gold_hi
                col = _blend(_PILL_BG, wave_color,
        ...
        if active:
            ...
            if self.glass.ok:
                ...
                try:
                    ...
                    # Show the companion bar above the glass pill (VAL-MAC-034).
                    if drawn and getattr(self, "widget", None):
                        try:
                            tx, ty = self.glass._tk_geo or (self._mx, self._my)
                            self.widget.render_and_push(self.frame)
                            self.widget.show_above(tx, ty, 380)
                        except Exception:
                            pass
                except Exception as e:
            ...
            if not drawn:
                ...
                if not self.visible:
                    self._show()
                    # Show companion bar above the canvas fallback (VAL-MAC-034).
                    if getattr(self, "widget", None):
                        try:
                            self.widget.render_and_push(self.frame)
                            self.widget.show_above(self._mx, self._my, W)
                        except Exception:
                            pass
            ...
            self._hide_all()
            # Hide the companion bar when the island is idle.
            if getattr(self, "widget", None):
                try:
                    self.widget.hide()
                except Exception:
    ...
    def _tick(self):
        ...
        # Synced from Windows canonical overlay.py.
        if self.state == "idle":
            try:
        ...
        # window is unfocused (bugfix-island-freeze).
        self.frame += 1
        ...
        try:
            ...
            if self.state == "done":
                ...
                # A legacy mode-suggestion chip freezes while a mode is active.
                # A plain paste hint keeps counting down normally.
                if not (self.is_suggest and self.armed):
                    self.hint_left -= 1
                if self.hint_left <= 0:
                    self.state = "idle"
            if self.state in ("listening", "search") and self.state != prev_state:
    ...
    def ask_questions(self, questions, on_complete, animate=True):
        ...
        try:
            ...
            win.attributes("-topmost", True)
            try:
                win.attributes("-transparent", True)  # macOS clear window
                win.configure(bg=_CLEAR)
            except tk.TclError:
                win.configure(bg=_TRANSPARENT)
            canvas = tk.Canvas(win, bg=_CLEAR, highlightthickness=0, bd=0)
    ...
    def show_deck(self, items, on_complete=None, on_fav=None):
        """The History deck — Mumble's mini dashboard (default Ctrl+Option+H). ONE surface unifying:
        ...
        try:
            ...
            for mkey in ("prompt", "email", "list", "reply", "foreign"):
            ...
            def _badge(it):
                if it.get("image_path"):
                    return "IMG", MODE_COLORS.get("list", "#3B82F6")
                src = it.get("source", "")
                if src == "transcript":
                    m = it.get("mode", "text")
            ...
            def _row_click(e, it, i):
                ...
                if e.x > w - 30:
                    ...
                    return
                state["sel"] = i
                if (e.state & 0x8) or job_mode():  # Cmd held (macOS), or job mode
                    _toggle_check(i)
                else:
            ...
            def _render():
                ...
                def hit(it):
                    return (not q or q in (it.get("text") or "").lower()
                            ...
                            or (it.get("image_path") and q in "image"))
            ...
            for w in (win, entry):
                ...
                w.bind("<Command-space>", lambda e: _toggle_check())
