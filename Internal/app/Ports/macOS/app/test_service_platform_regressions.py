...
def test_updater_installer_and_autostart():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        os.makedirs(nested)
        open(os.path.join(nested, "mumble.py"), "w").close()
        ...
        try:
            update.sys.platform = "darwin"
            update._write_swap_script(td, os.path.join(td, "new"),
                                      os.path.join(td, "app"))
        finally:
            update.sys.platform = old_platform
        script = open(os.path.join(td, "apply_update.command"),
                      encoding="utf-8").read()
