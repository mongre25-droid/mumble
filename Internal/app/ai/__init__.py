...
def _polish_long(
    raw, api_key, model, url, aggressiveness, *, route_decision=None
):
...
def cerebras_polish(
    ...
    processing_route.require_text_shaping(
        ...
        expected_lane="text",
        expected_provider=_provider_for_url(url),
        api_key=api_key,
        model=model,
    )
    if not second_opinion and len((raw or "").split()) >= 800:
        return _polish_long(
            raw, api_key, model, url, aggressiveness,
            route_decision=route_decision,
        )
...
def _polish_chunk(
    raw, api_key, model, url, aggressiveness, timeout=120,
    route_decision=None,
):
...
def polish_text(raw, api_key, model="gpt-oss-120b", url=CEREBRAS_URL,
    ...
    for ch in _split_for_polish(raw, max_words_per_chunk):
        t, trunc = _polish_chunk(
            ch, api_key, model, url, aggressiveness,
            route_decision=route_decision,
        )
        if t:
            out.append(t)
...
def _provider_for_url(url):
    """Resolve the provider identity that a frozen route must authorize."""
...
def cerebras_warm(
    api_key, model="gpt-oss-120b", timeout=20, url=CEREBRAS_URL,
    route_decision=None,
):
...
def synthesize_with_fallback(text, voice_id=None, model=None,
    ...
    # One invocation authorizes one exact provider and model.
