...
from mumble import SAMPLE_RATE, Mumble
...
def make_obj(transcribe, results=None, trace_events=None):
    ...
    if trace_events is not None:
        ...
        def trace_once(event, **fields):
            if event not in seen:
                seen.add(event)
                trace_events.append((event, fields))

        obj._trace_mark_once = trace_once
    return obj
...
trace_events = []
obj = make_obj(fake_transcribe, trace_events=trace_events)
...
check("first stable partial is measured exactly once",
      [name for name, _fields in trace_events] == ["stable_partial_ready"])
check("partial timing evidence contains no dictated text",
      all("text" not in fields for _name, fields in trace_events))
