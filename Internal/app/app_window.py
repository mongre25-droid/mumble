...
class AppWindow:
    ...
    def _build_home(self, b):
        ...
        hk = self._hotkey_pretty(self.settings.get("hotkey", "ctrl+windows"))
        ...
        for keys, what in [
            (
                self._hotkey_pretty(self.settings.get("hotkey", "ctrl+windows")),
                "Start / stop dictating",
            ),
            (
                self._hotkey_pretty(
                    self.settings.get("quick_paste_hotkey", "ctrl+alt+v")
                ),
        ...
        for key, name, desc, example in [
            (
                "text",
                "Text",
                "Clean dictation — fixes punctuation, removes “um / uh”, and obeys "
                "“new line” and “bullet point”.",
                "just start talking",
            ),
            (
                "email",
        ...
        for t in [
            ...
            "Press Ctrl + Alt + D — History opens with your recent transcripts, "
        ...
        ui.label(
            pro,
            "Pro Mode is powered by Cerebras (gpt-oss-120b), Mumble's recommended "
            "low-latency service. Its current rate-limited free tier uses your own API key "
            "(no built-in key, nothing hard-coded). "
        ...
        for step in [
            ...
            "2.  Review the current free-tier limits, then sign up.",
        ...
        for t in [
            ...
            "a Cerebras API key; a rate-limited free tier is currently available.",
    ...
    def _build_settings(self, b):
        ...
        self._hotkey_card(
            ...
            "For the full History window use Ctrl + Alt + D.",
            "quick_paste_hotkey",
        ...
        self._hotkey_card(
            b,
            "Web Search hotkey",
            "Prepare selected or dictated words, explain the chosen online "
            "provider, and wait for Search online before opening the browser.",
            "web_search_hotkey",
    ...
    def _build_share_about(self, b):
        ...
        for n, t in [
            (
                "1",
                "Send them the file “Mumble.zip” — email, WhatsApp, Drive, a USB stick.",
            ),
            ("2", "They right-click it → “Extract All” → run “Install Mumble.bat”."),
    ...
    def _build_pro_card(self, b):
        ...
        ui.label(
            ...
            "in about half a second. Your audio is always transcribed on your PC and "
        ...
        ui.label(
            card,
            "API keys are mandatory: Pro Mode needs your own Cerebras key (a rate-limited "
            "free tier is currently available; there is "
        ...
        ui.label(
            card,
            "Cloud AI runs on Cerebras (spelled C-E-R-E-B-R-A-S). Review its current "
            "free-tier limits before starting. "
        ...
        # ---- AI provider: same two processing routes as the main settings. ----
        import ai as _ai
    ...
    def _build_mode_button_card(self, b):
        """The held 'mode key' that arms mode detection + the AI second-opinion toggle."""
        card = self._card(b)
        ui.label(card, "Mode button", size=12, semibold=True, bg=C.surface).pack(
            anchor="w"
        )
        ui.label(
            card,
            "Hold this key WHILE you say a mode keyword (prompt, email, list, reply, "
            "foreign, convert) to switch modes. Let go — or never press it — "
            "and everything is treated as plain dictation. Right-Shift is recommended: "
            "unlike Right-Ctrl it does NOT overlap the Ctrl+Windows record hotkey, so "
            "recording can never arm a mode by accident. Avoid any key used in your "
            "record hotkey.",
            size=10,
            color=C.text_mute,
            bg=C.surface,
        ...
        self._row(
            card,
            "Use the mode button",
            "Off = no mode detection at all (pure dictation).",
            lambda h: ui.Switch(
                h,
                value=self.settings.get("mode_button_enabled", True),
                bg=C.surface,
                command=self.ctrl.set_mode_button_enabled,
            ).pack(),
        )

        row = tk.Frame(card, bg=C.surface)
        row.pack(anchor="w", fill="x", pady=(6, 0))
        mk_var = tk.StringVar(value=self.settings.get("mode_key", "right ctrl"))
        e = _entry(row, mk_var, width=20)
        e.pack(side="left", ipady=5)
        mk_msg = ui.label(card, "", size=10, color=C.text_mute, bg=C.surface)

        def apply_key():
            ok, m = self.ctrl.apply_mode_key(mk_var.get().strip())
            mk_msg.configure(text=m, fg=C.gold if ok else C.red)

        ui.RoundButton(row, "Apply", apply_key, kind="primary", bg=C.surface).pack(
            side="left", padx=(10, 0)
        )

        # Click-to-capture rebind: press the key you want to hold for modes, and it's
        # recorded + applied — no need to type the key name.
        def capture_key():
            cap_btn.set_text("Press a key / button…")
            mk_msg.configure(
                text="Press the key OR mouse button you want to hold for modes…",
                fg=C.text_mute
            )
            self.win.update_idletasks()
            ...
            def worker():
                ...
                name = ""
                try:
                    # Captures a single key OR a mouse button (side buttons,
                    # middle, etc.) — whichever the user presses first.
                    import bindings as _bindings
                    name = _bindings.capture(timeout=15.0)
                except Exception as ex:
                    print("key capture error:", ex)
                finally:
                ...
                def done():
                    ...
                    if name:
                        mk_var.set(name)
                        ok, m = self.ctrl.apply_mode_key(name)
                        mk_msg.configure(text=m, fg=C.gold if ok else C.red)
                    else:
                        mk_msg.configure(
                            text="Didn't catch anything — type it instead "
                                 "(e.g. mouse:x2).", fg=C.red
                        )
        ...
        cap_btn = ui.RoundButton(row, "Press a key / button", None, kind="ghost",
                                 bg=C.surface)
        cap_btn.command = capture_key
        cap_btn.pack(side="left", padx=(8, 0))

        ui.label(
            row,
            "e.g. right shift, right alt, or a mouse side-button (mouse:x2) — "
            "or click “Press a key / button”",
            size=9,
            color=C.text_mute,
            bg=C.surface,
        ).pack(side="left", padx=(10, 0))
        mk_msg.pack(anchor="w", pady=(8, 0))

        # Live preview of the current mode key, drawn as a keycap-style button so it's
        # obvious it's a physical key to hold (the "Shift as a button" requirement).
        keyrow = tk.Frame(card, bg=C.surface)
        keyrow.pack(anchor="w", fill="x", pady=(8, 0))
        ui.label(
            keyrow, "Hold to switch modes:", size=10, color=C.text_dim, bg=C.surface
        ).pack(side="left")
        self._keycap(
            keyrow, self._hotkey_pretty(self.settings.get("mode_key", "right shift"))
        ).pack(side="left", padx=(8, 0))
    ...
    def _build_prompt_prefs_card(self, b):
        ...
        self._row(
            ...
            "How much plain dictation (no mode key) is cleaned. Light = smallest fixes, "
    ...
    def _build_vocabulary_card(self, b):
        ...
        initial_vocab_text = self._format_vocabulary_text(terms, vocab)
        self.vocab_text.insert("1.0", initial_vocab_text)
        # Keep the persisted snapshot separate from the live editor. Island
        # learn/undo events can then be merged without erasing unsaved typing.
        self._vocab_baseline_terms = list(terms)
        self._vocab_baseline_pairs = dict(vocab)
        self._vocab_baseline_text = initial_vocab_text
        row = tk.Frame(card, bg=C.surface)
        ...
        self.vocab_msg = ui.label(
            card, "", size=9, color=C.text_mute, bg=C.surface, wrap=530)
        self.vocab_msg.pack(anchor="w", pady=(6, 0))

        # Experimental correction learning — classic-UI fallback. The live web
        # Settings card offers the same controls; keeping them here means a user
        # whose WebView is unavailable can still opt in and manage the feature.
        learn = tk.Frame(card, bg=C.surface2, highlightthickness=1,
                         highlightbackground=C.border, padx=12, pady=10)
        learn.pack(fill="x", pady=(12, 0))
        lhead = tk.Frame(learn, bg=C.surface2)
        lhead.pack(fill="x")
        ui.label(lhead, "Experimental · Learn from corrections", size=10,
                 semibold=True, color=C.gold_hi, bg=C.surface2).pack(side="left")
        self.correction_switch = ui.Switch(
            lhead,
            value=bool(self.settings.get("correction_learning_enabled", False)),
            command=self._toggle_correction_learning,
            bg=C.surface2,
        )
        self.correction_switch.pack(side="right")
        ui.label(
            learn,
            "After plain dictation the island offers Fix. Edit only the word or "
            "short phrase Mumble misheard; the exact rule is previewed, learned "
            "locally and can be undone.",
            size=9, color=C.text_mute, bg=C.surface2, wrap=530,
        ).pack(anchor="w", pady=(4, 7))

        # Auto-detection is a distinct privacy choice. Keep its current state and
        # limits visible beside the main experiment switch so enabling correction
        # learning never implies undisclosed observation of a target application.
        detect = tk.Frame(learn, bg=C.surface2)
        detect.pack(fill="x", pady=(2, 9))
        detect_text = tk.Frame(detect, bg=C.surface2)
        detect_text.pack(side="left", fill="x", expand=True)
        ui.label(
            detect_text,
            "Detect corrections in the target app",
            size=9, semibold=True, color=C.text, bg=C.surface2,
        ).pack(anchor="w")
        ui.label(
            detect_text,
            "Windows only: watches only the field Mumble just pasted into, for "
            "up to 30 seconds and in memory. Password fields, focus changes, and "
            "broad rewrites are ignored. Every correction still needs your "
            "approval on the island.",
            size=8, color=C.text_mute, bg=C.surface2, wrap=450,
        ).pack(anchor="w", pady=(2, 0))
        self.correction_auto_detect_switch = ui.Switch(
            detect,
            value=bool(self.settings.get(
                "correction_learning_auto_detect", True)),
            command=self._toggle_correction_learning_auto_detect,
            bg=C.surface2,
        )
        self.correction_auto_detect_switch.pack(
            side="right", anchor="n", padx=(10, 0))

        lbuttons = tk.Frame(learn, bg=C.surface2)
        lbuttons.pack(fill="x")
        ui.RoundButton(
            lbuttons, "Correct last dictation", self._open_correction_learning,
            kind="ghost", bg=C.surface2, height=28, font_size=9,
        ).pack(side="left")
        ui.RoundButton(
            lbuttons, "Undo learned correction", self._undo_correction_learning,
            kind="ghost", bg=C.surface2, height=28, font_size=9,
        ).pack(side="left", padx=(7, 0))
        self.correction_msg = ui.label(
            learn, "", size=8, color=C.text_mute, bg=C.surface2, wrap=520)
        self.correction_msg.pack(anchor="w", pady=(6, 0))

    def _toggle_correction_learning(self, value):
        enabled = self.ctrl.set_correction_learning_enabled(bool(value))
        auto_detect = bool(self.correction_auto_detect_switch.value)
        if enabled and auto_detect:
            message = "On - target-app detection on; island approval required"
        elif enabled:
            message = "On - manual Fix only"
        else:
            message = "Off"
        self.correction_msg.configure(
            text=message,
            fg=C.gold if enabled else C.text_mute,
        )

    def _toggle_correction_learning_auto_detect(self, value):
        enabled = self.ctrl.set_correction_learning_auto_detect(bool(value))
        experiment_on = bool(self.correction_switch.value)
        if enabled and experiment_on:
            message = "Auto-detect on - island approval is always required"
        elif enabled:
            message = "Auto-detect ready when the experiment is enabled"
        else:
            message = "Auto-detect off - manual Fix only"
        self.correction_msg.configure(
            text=message,
            fg=C.gold if (enabled and experiment_on) else C.text_mute,
        )

    def _open_correction_learning(self):
        result = self.ctrl.open_correction_learning() or {}
        self.correction_msg.configure(
            text=result.get("message", ""),
            fg=C.gold if result.get("ok") else C.red,
        )

    def _undo_correction_learning(self):
        result = self.ctrl.undo_correction_learning() or {}
        self.correction_msg.configure(
            text=result.get("message", ""),
            fg=C.gold if result.get("ok") else C.red,
        )
    ...
    def _save_presets(self):
        ...
        try:
            if not _presets.save_custom(custom):
                self.preset_count_lbl.configure(
                    text="Couldn't save custom presets. Check the data folder."
                )
                return
            self.preset_count_lbl.configure(
    ...
    @staticmethod
    def _parse_vocabulary_text(raw):
        """Parse the classic editor while retaining incomplete in-progress lines."""
        vocab, terms, incomplete = {}, [], []
        for line in raw.splitlines():
            ...
            if "=" in line:
                ...
                terms.append(line)
        return terms, vocab, incomplete

    @staticmethod
    def _format_vocabulary_text(terms, vocab, incomplete=()):
        lines = list(terms or [])
        lines.extend(f"{key} = {value}" for key, value in (vocab or {}).items())
        lines.extend(incomplete or [])
        return "\n".join(lines)

    @classmethod
    def _merge_vocabulary_text(cls, old_terms, old_pairs, current_text,
                               fresh_terms, fresh_pairs):
        """Three-way merge disk learning into a possibly dirty classic editor."""
        current_terms, current_pairs, incomplete = cls._parse_vocabulary_text(
            current_text)
        old_term_set = set(old_terms or [])
        current_term_set = set(current_terms)
        fresh_term_set = set(fresh_terms or [])

        # Preserve user-added/retained terms, while accepting external undo of
        # an unchanged baseline term.
        merged_terms = [
            term for term in current_terms
            if term not in old_term_set or term in fresh_term_set
        ]
        # Append newly learned terms that did not exist in the old baseline.
        for term in fresh_terms or []:
            if term not in old_term_set and term not in current_term_set:
                merged_terms.append(term)

        old_pairs = dict(old_pairs or {})
        fresh_pairs = dict(fresh_pairs or {})
        merged_pairs = {}
        keys = list(dict.fromkeys(
            [*old_pairs.keys(), *current_pairs.keys(), *fresh_pairs.keys()]
        ))
        for key in keys:
            old_has = key in old_pairs
            current_has = key in current_pairs
            fresh_has = key in fresh_pairs
            user_changed = (
                (not current_has or current_pairs[key] != old_pairs[key])
                if old_has else current_has
            )
            if user_changed:
                if current_has:
                    merged_pairs[key] = current_pairs[key]
            elif fresh_has:
                merged_pairs[key] = fresh_pairs[key]

        return cls._format_vocabulary_text(
            merged_terms, merged_pairs, incomplete)

    def _save_vocabulary(self):
        raw = self.vocab_text.get("1.0", "end-1c")
        terms, vocab, incomplete = self._parse_vocabulary_text(raw)
        # Pass the snapshot this editor started from. The controller applies
        # only the user's delta to the latest locked disk value, so a correction
        # learned between opening Settings and clicking Save is never erased.
        self.ctrl.set_vocabulary(
            vocab,
            terms,
            getattr(self, "_vocab_baseline_pairs", {}),
            getattr(self, "_vocab_baseline_terms", []),
        )
        # The atomic controller write may also have preserved a rule learned by
        # another process moments ago. Pull that final snapshot back through the
        # same three-way merge so the editor and its baseline cannot go stale.
        ...
        if incomplete:
            ...
            msg += f" Skipped {count} incomplete “wrong = right” line{'s' if count != 1 else ''}."
        self.vocab_msg.configure(text=msg)

    def refresh_vocabulary(self):
        """Merge an island-learned/undone rule without erasing live edits."""
        if not hasattr(self, "vocab_text") or not self.vocab_text.winfo_exists():
            return
        fresh_terms = list(self.settings.get("vocabulary_terms", []) or [])
        fresh_pairs = dict(self.settings.get("vocabulary", {}) or {})
        fresh_text = self._format_vocabulary_text(fresh_terms, fresh_pairs)
        current_text = self.vocab_text.get("1.0", "end-1c")
        baseline_text = getattr(self, "_vocab_baseline_text", current_text)
        dirty = current_text != baseline_text

        if dirty:
            updated_text = self._merge_vocabulary_text(
                getattr(self, "_vocab_baseline_terms", []),
                getattr(self, "_vocab_baseline_pairs", {}),
                current_text,
                fresh_terms,
            ...
            updated_text = fresh_text

        if updated_text != current_text:
            self.vocab_text.delete("1.0", "end")
            self.vocab_text.insert("1.0", updated_text)
        self._vocab_baseline_terms = fresh_terms
        self._vocab_baseline_pairs = fresh_pairs
        self._vocab_baseline_text = fresh_text
        if dirty:
            self.vocab_msg.configure(
                text=("Kept your unsaved edits and merged the learned vocabulary "
                      "change. Review, then Save."),
                fg=C.gold,
            )
    ...
    def _build_search_engine_card(self, b):
        """Choose the provider used for an explicit Web Search."""
        card = self._card(b)
        ui.label(card, "Search engine", size=12, semibold=True,
                 bg=C.surface).pack(anchor="w")
        ui.label(
            card,
            "Used only after you explicitly confirm Search online. Mumble Find "
            "always keeps local results separate.",
        ...
        _LABELS = [("Google", "google"), ("Perplexity", "perplexity"),
                   ("Brave", "brave")]
        cur = (self.settings.get("search_engine", "perplexity") or
               "perplexity").lower()
        cur_label = next((lbl for lbl, v in _LABELS if v == cur), "Perplexity")
        ...
        def on_select(_e=None):
            engine = dict(_LABELS).get(var.get(), "perplexity")
    ...
    def _hotkey_card(self, b, title, desc, key, apply_fn):
        ...
        ui.label(
            card,
            desc + "  Examples: ctrl+windows, ctrl+alt+v — or a mouse "
    ...
    def _reset_settings(self):
        ...
        if self.ctrl.settings.reset_all() is False:
            try:
                self.mic_saved_msg.configure(
                    text="Could not reset settings — existing values were kept",
                    fg=C.red,
                )
            except Exception:
                pass
