...
def test_sign_out_clears():
    ...
    s = MockSettings(supabase_url="https://test.supabase.co",
                     supabase_anon_key="test-key")
    cs = CloudSync(s)
    with tempfile.TemporaryDirectory(prefix="mumble_signout_test_") as directory:
        isolated_session = os.path.join(directory, ".session")
        cs._storage = _FileStorage(isolated_session)

        # Create a fake session file without touching the real app-data path.
        with open(isolated_session, "w") as f:
            json.dump({"supabase.auth.token": "fake-token"}, f)

        check("session file exists before sign_out",
              os.path.exists(isolated_session))

        cs.sign_out()
        check("session file removed after sign_out",
              not os.path.exists(isolated_session))
...
def test_syncable_settings_strip():
    ...
    check("platform-specific hotkeys excluded",
          all(k in _SETTINGS_SYNC_EXCLUDE for k in (
              "hotkey", "quick_paste_hotkey", "history_hotkey",
              "search_hotkey", "mode_key")))
