...
# Prompt history is shared by the controller and web window. A burst of durable
# writes can legitimately keep the other writer waiting for more than the
# generic 3-second lock window on a busy Windows disk.
...
class PromptHistory:
    def __init__(self, history_path=None):
        ...
        # Prompt Memory was removed. Delete its separate continuity store while
        # preserving prompts.json, which is the user's visible Deck history.
    ...
    def _append_history(self, timestamp, request, prompt):
        with self._history_lock, exclusive_file_lock(
                self.history_path, timeout=WRITE_LOCK_TIMEOUT) as acquired:
            if not acquired:
                return False
            try:
