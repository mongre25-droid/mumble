...
**Research date:** 24 July 2026 (revalidated against baseline `0aca0233`; approved Issue #19 lanes refreshed from official sources on 26 July 2026)
...
### Issue #19 official-source refresh — 26 July 2026

This refresh is **upstream research evidence only**. No model or package was downloaded or executed, and no Mumble accuracy, latency, memory, recovery, offline, Windows-package, or user-acceptance result was produced. The official repository, release, model-card, and artifact metadata below were accessed on **26 July 2026**.

| Lane | Reproducible upstream pin and current fact | Issue #19 boundary |
|---|---|---|
| `faster-whisper` baseline | The current official release remains [`v1.2.1`](https://github.com/SYSTRAN/faster-whisper/releases/tag/v1.2.1); current official source is [`ed9a06cd`](https://github.com/SYSTRAN/faster-whisper/tree/ed9a06cd89a93e47838f564998a6c09b655d7f43). Mumble already pins `faster-whisper==1.2.1`. | Freeze and measure the installed Mumble model/runtime combination. A matching upstream version is not a benchmark pass. |
| Moonshine English streaming | The latest official release is [`v0.0.73`](https://github.com/moonshine-ai/moonshine/releases/tag/v0.0.73), source commit [`3df786f9`](https://github.com/moonshine-ai/moonshine/tree/3df786f968582008bdecc66ca365a60203f91260). Its self-contained Windows transcriber archive is 360,363,995 bytes with [release-published SHA-256](https://api.github.com/repos/moonshine-ai/moonshine/releases/tags/v0.0.73) `2adf58571dc71240d3035171f2d378f30838b506c28d9ced07f8507b28654dd3`. The released Python downloader accepts an optional SHA-256, but its standard Small Streaming download call does not supply one and accepts an existing cached path when no hash is supplied. English code/models remain MIT; non-English model terms remain non-commercial. | Admit only English Small Streaming, isolated from Mumble's main process. A Mumble-owned manifest must supply per-component SHA-256 values and prove corrupt-cache recovery, offline restart, missing-model handling, and baseline fallback before a safe run. |
| sherpa-onnx English streaming | Current official runtime source is [`546df6f9`](https://github.com/k2-fsa/sherpa-onnx/tree/546df6f963ae719dddd8b8d10749e9d9086b0d86), while the experiment must use Mumble's pinned `sherpa-onnx==1.13.4`. The selected model remains revision [`672fbf1`](https://huggingface.co/csukuangfj/sherpa-onnx-streaming-zipformer-en-2023-06-26/tree/672fbf1b30579d6585301139bb363f42a0ad4a24): a LibriSpeech-trained Icefall model converted for sherpa-onnx. The left-context-128 INT8 encoder/decoder/joiner total 72,649,734 bytes; the BPE model adds 244,865 bytes. Hugging Face publishes SHA-256 LFS object IDs for those four artifacts. | Keep runtime, three ONNX weights, BPE/token files, original Icefall checkpoint/training recipe, conversion, and Apache-2.0 notices as separate manifest entries. Published file metadata is supply-chain input, not Windows or quality evidence. |
| Parakeet quality ceiling | Official model revision remains [`7c35754`](https://huggingface.co/nvidia/parakeet-tdt-0.6b-v3/tree/7c35754d166cca382ad1e53e68b01e7c575f3a1d). `model.safetensors` is 2,508,311,120 bytes with published SHA-256 `3a2026366188c8c68598edbbff92f8d11590a08e0ae2e6775544e7b07d6a5e11`; `tokenizer.json` is a separate 1,159,960-byte artifact. The card identifies Granary and `nemo/asr-set-3.0` as training sources, 25 European languages, automatic language detection, punctuation/capitalisation, timestamps, and CC-BY-4.0 terms. It now also links a NeMo chunked-streaming recipe using 2-second chunks, 2 seconds of right context, and 10 seconds of left context. | Issue #19 admits this only as a strong-hardware completed-utterance quality ceiling. The upstream batch/chunk recipe does not establish Mumble first-stable-partial latency, recovery, Windows packaging, or resource fit. No large-model run is permitted without the declared hardware and safe reproducible package rights. |
| Lightweight local instructions | Official model/GGUF revisions remain Qwen3 0.6B [`23749fe`](https://huggingface.co/Qwen/Qwen3-0.6B-GGUF/tree/23749fefcc72300e3a2ad315e1317431b06b590a), Qwen2.5 1.5B Instruct [`91cad51`](https://huggingface.co/Qwen/Qwen2.5-1.5B-Instruct-GGUF/tree/91cad51170dc346986eccefdc2dd33a9da36ead9), and Granite 3.3 2B Instruct [`7cdf86c`](https://huggingface.co/ibm-granite/granite-3.3-2b-instruct-GGUF/tree/7cdf86ccd1f1bb3491c9b7017b033f2e51367397). Their declared Q8/Q4_K_M artifacts are respectively 639,446,688, 1,117,320,736, and 1,545,303,328 bytes, with Hugging Face-published SHA-256 values `9465e63a22add5354d9bb4b99e90117043c7124007664907259bd16d043bb031`, `6a1a2eb6d15622bf3c96857206351ba97e1af16c30d7a74ee38970e434e9407e`, and `ac71e9e32c0bea919b409c5918f69ca74339854b0319c5065e4e9fb6d95c4852`. All three official model repositories declare Apache-2.0. | Test Prompt, Email, Reply, and classification separately through one pinned runtime; retain deterministic text shaping as the control. Official instruction-following claims do not establish fidelity, abstention, or safe classification. |

The selected sherpa left-context-128 artifacts have these official Hugging Face LFS SHA-256 values: encoder `563fde436d16cf7607cf408cd6b30909819d03162652ef389c2450ced3f45ac1`, decoder `98da299f471e38bb4e1a8df579b8cc9122d6039576a77e357b3c60f17dd83b02`, joiner `d944208d660d67c8d72cd2acaeac971fa5ceb8c80e76c1968148846fedd6e297`, and BPE model `c53433de083c4a6ad12d034550ef22de68cec62c4f58932a7b6b8b2f1e743fa5`. The 5,048-byte `tokens.txt` is an ordinary Git object rather than an LFS artifact; a benchmark manifest must calculate and pin its content SHA-256 before any run.

For the instruction lane, the current official `llama.cpp` release is [`b10107`](https://github.com/ggml-org/llama.cpp/releases/tag/b10107), source commit [`c0bc8591`](https://github.com/ggml-org/llama.cpp/tree/c0bc8591e8815c63cb01dd3f051a8b0df02501c9). Its CPU-only Windows x64 archive is 18,213,827 bytes with [release-published SHA-256](https://api.github.com/repos/ggml-org/llama.cpp/releases/tags/b10107) `52133a0a5a8f6035b1bdd2f89c3425ea8b742413d9bdb9a2dee30e3a1681b18c`. This is a reproducible research pin only; Issue #19 must still test process failure, cancellation, idle cost, offline start, and baseline fallback.

The provenance boundary remains explicit: Qwen's and IBM's official GGUF repositories make the conversion publisher the model owner, but that does not remove the need to preserve the source model card, embedded tokenizer/template, conversion revision, licence, attribution, and branding. Granite's card describes permissively licensed public data plus internally generated synthetic data and also points to inherited limitations involving select proprietary data; Qwen's cards do not provide a dataset-by-dataset training ledger. Those are recorded provenance limits, not Mumble benchmark findings.
...
- **Parakeet as live streaming** is not admitted. The official v3 card now links a chunked-streaming recipe, but it does not establish Mumble's first-stable-partial, correction, recovery, resource, or Windows-package gates; Issue #19 keeps Parakeet in the completed-utterance quality-ceiling lane.
...
### Issue #19 implementation receipt â€” 26 July 2026

The first reproducible boundary now exists under `Development Files/Research/local-ai-benchmark/`:

- `v1/corpus-v1.json` is a seven-fixture synthetic manifest. Speech text is declared but audio is deliberately not materialised because no approved reproducible synthesizer or consented corpus is present. Owner-private recordings are explicitly prohibited without a separate consent record.
- `v1/gates-v1.json` freezes the eight mandatory adoption gates and task-specific metrics before candidate execution. Any `fail`, `blocked`, `not_run`, `unknown`, or missing gate retains the lane baseline.
- `v1/candidates-v1.json` keeps faster-whisper and deterministic text as the controls, restricts each approved candidate to its declared lane, separates the nine licence/provenance fields, and records exact official artifacts where this could be done without a large model download. The Moonshine English component set remains an explicit pre-run blocker rather than a guessed identity. Each faster-whisper run must record its exact local snapshot; the preliminary run below does so for the already-cached owner-configured `small.en` snapshot.
- `Development Files/Tooling/local_ai_benchmark.py` validates those inputs, calculates content-free WER/timing/resource records, runs the existing deterministic Prompt/Email/Reply/classification controls, and owns the cached-only Windows SAPI/faster-whisper reproduction command. Completed candidates use exact metric schemas and valid numeric domains; accuracy, latency, and resource gates are derived from measured thresholds; language, safety, licence, Windows, and packaging open contained candidate/gate-specific proof records and verify their hashes. Instruction candidates cannot advance without independent Prompt, Email, Reply, and classification results. Cache preparation enforces exact size/SHA-256, HTTPS through the final redirect, a hard byte limit, offline reuse, corrupt-cache recovery, atomic replacement, and declared fallback. The SAPI command binds the supplied directory to the committed `small.en` model identity, revision, exact file set, sizes, and hashes before measurement.
- `runs/2026-07-26-windows-z1/` records the exact Windows/AMD Z1 Extreme software and hardware tier plus the availability decision for every model candidate. It excludes machine names, usernames, serial numbers, stable device identifiers, audio, and transcript content.

Eleven focused tests passed. The validator accounted for nine candidates and seven fixtures. The deterministic fixture smoke reached all four simple text lanes with full fixture success and no classifier execution authority, but it did not run the predeclared repetition/failure-recovery matrix and therefore has `harness_smoke_only` evidence with adoption gates not evaluated. The added regressions prove that a completed candidate needs valid, exact-schema measured evidence plus five existing contained/hash-verified proof records, missing instruction modes block eligibility, impossible metric domains and extra content fields are rejected, false actions are calculated, non-Boolean recovery cannot become a pass, oversized downloads stop at the declared boundary, an HTTPS redirect cannot downgrade, and the cached baseline cannot be relabelled from another model.

One supervisor-requested preliminary faster-whisper check used no network access and no owner recording. Windows SAPI generated fixture `speech-dictation-001` with Microsoft Hazel Desktop; the temporary WAV was hashed and then deleted. The already-cached `Systran/faster-whisper-small.en` snapshot resolved to revision `d1d751a5f8271d482d14ca55d9e2deeebbae577f`; all four file sizes and SHA-256 values are in `runs/2026-07-26-windows-z1/preliminary-faster-whisper-small.en.json`. On this one 4.658-second synthetic utterance, faster-whisper 1.2.1/CTranslate2 4.8.0 CPU INT8 measured WER `0.20`, stop-to-final `2965.109 ms`, real-time factor `0.636519`, peak RAM `702.246 MB`, resident idle RAM `318.977 MB`, and one-second resident idle CPU `1.6%`. The completed-utterance baseline exposed no first stable partial, failure recovery was not run, and the 30-run matrix was not run. The committed cached-only command was then checked once after review correction: it reproduced the exact audio hash, WER, and retained-baseline decision, while its separate timing was not added to the benchmark population. These figures remain a small preliminary local measurement, not an accuracy conclusion, installed-package result, or adoption evidence.

The formal faster-whisper matrix remains `not_run` with the preliminary receipt linked separately. Moonshine, sherpa, Parakeet, Qwen3, Qwen2.5, Granite, and Model2Vec are also `not_run` for their exact missing runtime, weight, hardware, repeated-run, licence, or packaging conditions. No candidate advanced and both baselines were retained.

This is Mumble benchmark evidence, not upstream research evidence. Conversely, upstream file sizes, hashes, licences, and documented capabilities in the refreshed candidate table are supply-chain inputs, not Mumble accuracy, latency, resource, Windows, or package results. No paid-cloud comparison, model integration, large-model download, package build, or physical cross-platform run occurred.
...
Sources were first read on 24 July 2026. The approved Issue #19 lanes were refreshed from official sources on 26 July 2026. GitHub repositories, releases, and Hugging Face model cards are pinned below where a commit or model revision was available.

### Speech, streaming, and VAD

- [faster-whisper release `v1.2.1`](https://github.com/SYSTRAN/faster-whisper/releases/tag/v1.2.1) and [source at `ed9a06cd`](https://github.com/SYSTRAN/faster-whisper/tree/ed9a06cd89a93e47838f564998a6c09b655d7f43) — current official baseline release/source identity and MIT runtime licence.
- [Moonshine Voice release `v0.0.73`](https://github.com/moonshine-ai/moonshine/releases/tag/v0.0.73), [source at `3df786f9`](https://github.com/moonshine-ai/moonshine/tree/3df786f968582008bdecc66ca365a60203f91260), and [released downloader](https://github.com/moonshine-ai/moonshine/blob/3df786f968582008bdecc66ca365a60203f91260/python/src/moonshine_voice/download_file.py) — streaming architecture, Windows assets and release digests, model-download integrity boundary, and version identity.
- [Moonshine Voice licence at `3df786f9`](https://github.com/moonshine-ai/moonshine/blob/3df786f968582008bdecc66ca365a60203f91260/LICENSE) — MIT code/English model terms and separate non-commercial non-English model terms.
- [Moonshine v2 paper](https://arxiv.org/abs/2602.12241) — model architecture and upstream evaluation context.
- [sherpa-onnx repository at `546df6f9`](https://github.com/k2-fsa/sherpa-onnx/tree/546df6f963ae719dddd8b8d10749e9d9086b0d86) — current cross-platform runtime source, streaming/Windows support, and Apache-2.0 licence.
...
- [NVIDIA Parakeet TDT 0.6B v3 model card at `7c35754`](https://huggingface.co/nvidia/parakeet-tdt-0.6b-v3/blob/7c35754d166cca382ad1e53e68b01e7c575f3a1d/README.md) — languages, timestamps, punctuation/capitalisation, evaluation, NeMo chunked-streaming recipe, training sources, and CC-BY-4.0 terms.
...
- [`llama.cpp` release `b10107`](https://github.com/ggml-org/llama.cpp/releases/tag/b10107) and [source at `c0bc8591`](https://github.com/ggml-org/llama.cpp/tree/c0bc8591e8815c63cb01dd3f051a8b0df02501c9) — pinned Windows CPU runtime archive/digest, GGUF support, grammar constraints, and MIT licence.
- [Official Qwen3 0.6B model card at `c1899de`](https://huggingface.co/Qwen/Qwen3-0.6B/blob/c1899de289a04d12100db370d81485cdf75e47ca/README.md) and [official GGUF at `23749fe`](https://huggingface.co/Qwen/Qwen3-0.6B-GGUF/tree/23749fefcc72300e3a2ad315e1317431b06b590a) — languages, inference modes, Apache-2.0 terms, and 639.4 MB Q8 artifact.
- [Official Qwen2.5 1.5B Instruct model card at `989aa79`](https://huggingface.co/Qwen/Qwen2.5-1.5B-Instruct/blob/989aa7980e4cf806f80c7fef2b1adb7bc71aa306/README.md) and [official GGUF at `91cad51`](https://huggingface.co/Qwen/Qwen2.5-1.5B-Instruct-GGUF/tree/91cad51170dc346986eccefdc2dd33a9da36ead9) — instruction/language scope, Apache-2.0 terms, and 1.117 GB Q4_K_M artifact.
- [IBM Granite 3.3 2B Instruct model card at `707f574`](https://huggingface.co/ibm-granite/granite-3.3-2b-instruct/blob/707f574c62054322f6b5b04b6d075f0a8f05e0f0/README.md) and [official GGUF at `7cdf86c`](https://huggingface.co/ibm-granite/granite-3.3-2b-instruct-GGUF/tree/7cdf86ccd1f1bb3491c9b7017b033f2e51367397) — languages, evaluation, training-provenance limits, Apache-2.0 terms, and 1.545 GB Q4_K_M artifact.
