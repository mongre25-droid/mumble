...
**26 July 2026 implementation note:** the two bullets above describe the historical `0aca0233` revalidation point. At Issue #19 parent `65f27576f093553418540abe58501043971e8b1f`, accepted Issue #16 durability and content-free timing seams are present and are reused rather than redesigned. Issue #19 now has a versioned synthetic manifest, predeclared gates, exact host record, content-free scoring/ledger functions, and hash/cache/fallback tests. One already-cached `small.en` snapshot was hash-proved and measured offline against a temporary Windows SAPI utterance; that single preliminary receipt records WER, stop-to-final, real-time factor, RAM/VRAM, model size, idle cost, and the unavailable first-stable-partial/failure-recovery fields. The formal 30-run faster-whisper matrix and every downloaded-model candidate remain `not_run`. This updates implementation status without converting one synthetic utterance, upstream documentation, or local harness smoke into full speech, package, or adoption evidence.
