...
async function call(name, ...args) {
  ...
  switch (name) {
    ...
    case "check_updates":
      return {
        status: "uptodate",
        message: "You're on the latest version (v0.9). (Preview)",
      };
    ...
    case "meeting_extract_decisions": {
      ...
      mkd.decision_count = mkd.key_decisions.length;
    ...
    case "meeting_extract_questions": {
      ...
      mq.question_count = mq.open_questions.length;
    ...
    case "meeting_extract_deep": {
      ...
      mdp.decision_count = mdp.key_decisions.length;
      mdp.question_count = mdp.open_questions.length;
...
async function bootHome() {
  ...
  const hk = await call("get_hotkeys");
...
let HX = {
  ...
  prompts: [],
  favorites: [],
  // Filter chips: each content type is a key; true = chip active (visible).
  // All chips active by default to show all content types in the unified view.
  // Empty Set = all chips deselected = show all (default behaviour).
  activeFilters: new Set(["transcripts", "clipboard", "prompts", "favorites", "emails"]),
  q: "",
  sort: "new",
  pre: false,
...
function drawFilterChips() {
  const host = $("#hist-filterchips");
  if (!host) return;
  const counts = {
    transcripts: (HX.transcripts || []).filter((i) => (i.mode || "text") !== "email").length,
    clipboard: (HX.clipboard || []).length,
    prompts: (HX.prompts || []).length,
    favorites: (HX.favorites || []).length,
    emails: (HX.transcripts || []).filter((i) => (i.mode || "") === "email").length,
  };
  // Update count badges in-place without rebuilding buttons
  $$(".filterchip", host).forEach((b) => {
    const f = b.dataset.filter;
    const span = b.querySelector(".filterchip-count");
    const c = counts[f] || 0;
    if (span) span.textContent = c ? "(" + c + ")" : "";
    b.classList.toggle("active", HX.activeFilters.has(f));
    // When NO chips are active, treat as "show all" — all chips visually active
    if (HX.activeFilters.size === 0) b.classList.add("active");
  });
}

function toggleFilter(name) {
  if (HX.activeFilters.has(name)) {
    HX.activeFilters.delete(name);
  } else {
    HX.activeFilters.add(name);
  }
  // If ALL 5 chips are now active, that's equivalent to "show all" — clear the set
  if (HX.activeFilters.size === 5) {
...
async function copyLatest() {
  ...
  toast("Copied latest transcript", "ok", 1500);
}
...
function drawHistory() {
  ...
  const EMPTY = {
    transcripts: "Nothing here yet \u2014 press Ctrl+Win and dictate something.",
  ...
  if (!items.length) {
    if (HX.activeFilters.size === 1) {
      ...
      if (allEmpty) {
        emptyMsg = "Nothing here yet \u2014 press Ctrl+Win and dictate to get started.";
      } else {
...
// Ctrl+Alt+V / the homepage button → open History as the hub (and pin on top).
...
async function renderStats() {
  ...
  const wkChip = growthChip(ins.trend_pct, null);
  ...
  const tiles = [
    [
      ...
      wkChip
        ? wkChip + '<span class="tile-sub"> this week</span>'
  ...
  $$("[data-setting]").forEach((el) => {
    ...
    if (!el.dataset.wired) {
      ...
      el.addEventListener(ev, async () => {
        ...
        const r = await call("set_setting", key, v);
        setNested(SET, key, v);
        ...
        if (
          ...
          reflectCloudStt();
        if (key === "prompt_provider") renderAiModeFields();
        ...
        if (["pro_mode", "hardware_tier", "english_only", "model", "device",
             "hardware_tier_resolved"].includes(key)) {
          updateSetupSummary();
        }
        ...
        if (key.endsWith("_api_key")) {
          ...
          if (SET.prompt_provider === pr) renderAiModeFields();
...
const PROVIDER_DESC = {
  ...
  anthropic: "Official Anthropic Claude models.",
  openrouter: "One key, many models.",
  deepseek: "DeepSeek models — not recommended (slower, no free tier).",
  groq: "Fast open models — not recommended (slower).",
};
...
function updateSetupSummary() {
  ...
  const sumHw = $("#sum-hw");
  if (sumHw) {
    const tier = SET.hardware_tier_resolved || SET.hardware_tier || "auto";
    const tierNames = { auto: "Auto", weak: "Weak PC", mid: "Mid PC", powerful: "Powerful PC" };
    const tierName = tierNames[tier] || tier;
    const cpuThreads = SET.cpu_threads || 0;
    const device = SET.device || "auto";
    const hwDetail = device === "cuda" ? " · GPU" : (device === "cpu" ? " · CPU" : "");
    sumHw.textContent = tierName + hwDetail;
    sumHw.className = "setup-value";
  }
  // Loaded model
...
function syncOpenRouterKey(value) {
  ...
  // The prompting key field is rendered dynamically — refresh it if it's the OR key.
  if (SET.prompt_provider === "openrouter") {
    const ak = $("#aimode-key");
    if (ak && ak.value !== v) ak.value = v;
  }
}

/* AI Mode (prompting engine): the chosen provider gets its OWN key+model
   fields right here — no dead-end pointing at another card. The fields bind
   to the same stored settings the AI Provider card uses. */
function renderAiModeFields() {
  const host = $("#aimode-fields");
  if (!host) return;
  const p = SET.prompt_provider || "";
  if (!p) {
    host.innerHTML =
      '<p class="help">Prompt mode stays on your main provider. Pick a provider above to give prompting its own, stronger engine.</p>';
    return;
  }
  // Prompting providers (owner v0.9): Cerebras + OpenRouter ONLY. Every other
  // prompting option (OpenAI, DeepSeek, Claude, Groq, the retired Local LLM) was
  // removed to streamline the provider UX.
  const cfg = {
    cerebras: {
  ...
  host.innerHTML = `<div class="grid g2 gap12">
      <div class="field"><span class="label">${esc(cfg.label)}</span><input class="input selectable" type="password" id="aimode-key" placeholder="${esc(cfg.ph)}" value="${esc(SET[cfg.key] || "")}"></div>
      <div class="field"><span class="label">Model</span><select class="select" id="aimode-model"></select></div>
    </div>
    <div class="flex items-center justify-center gap12 mt10">
      <button class="btn btn-sm" id="aimode-test">Save &amp; test</button>
      <span class="feedback" id="aimode-fb"></span>
    </div>`;
  fillModelSelect($("#aimode-model"), p, false);
  $("#aimode-key").addEventListener("change", async (e) => {
    await call("set_setting", cfg.key, e.target.value);
    SET[cfg.key] = e.target.value;
    // Shared OpenRouter key: entering it here fills the main + Reader fields too.
    if (cfg.key === "openrouter_api_key") syncOpenRouterKey(e.target.value);
    fillModelSelect($("#aimode-model"), p, true); // new key → fetch the real list
  });
  $("#aimode-model").addEventListener("change", (e) => {
    call("set_setting", cfg.model, e.target.value);
    SET[cfg.model] = e.target.value;
  });
  $("#aimode-test").addEventListener("click", async () => {
    flash($("#aimode-fb"), "Testing…", "busy");
    const r = await call("test_key", p, $("#aimode-key").value.trim());
    flash($("#aimode-fb"), r.message, r.ok ? "ok" : "err");
  });
}
...
async function saveVocab(fb) {
  ...
  await call("set_setting", "vocabulary_terms", terms);
  await call("set_setting", "vocabulary", pairs);
  SET.vocabulary_terms = terms;
  SET.vocabulary = pairs;
  flash(
    $(fb),
    terms.length +
      " terms + " +
...
/* ============================================================================
   ...
   ========================================================================== */
let OB = { step: 1, total: 10, tier: "auto", lang: "en" };
function obShow(n) {
...
async function finishOnboarding() {
  ...
  // The Big Shift: persist the PC class + language choice so the controller
  // resolves the right on-device model. english_only is a boolean.
  const tier = $("#ob-hw .active")?.dataset.tier || OB.tier || "auto";
  ...
  await call("set_setting", "hardware_tier", tier);
...
/* Responsive SCALE-DOWN, not reflow (owner 2026-06-20: "anchor the layout, not
   the component sizes"). The app is built around a ~980px content column; on a
   narrower window we scale the WHOLE UI down proportionally with CSS `zoom`, so
   the layout structure is IDENTICAL on every screen — just smaller — instead of
   widgets wrapping into new rows and the hierarchy breaking. It NEVER scales up
   (wide windows still breathe to the 1280px max-width); a floor keeps a tiny
   window readable. WebView2 is Chromium, so `zoom` is supported and avoids the
   transform/footprint math a scale() would need. */
// The design width the layout targets. Refinement pass §11: this was 980, but the
// DEFAULT 960px window has an innerWidth of only ~930-945 (window chrome + an
// optional scrollbar), so 980 made applyResponsiveScale zoom the body to ~0.96 on
// EVERY launch — a visible shrink-flicker the instant the page loaded. Pinning it
// just below the default innerWidth means the default window renders at a true 100%
// (no zoom, no flicker); the UI only scales down once the user makes the window
// genuinely narrower than this.
const SCALE_DESIGN_W = 928;
function applyResponsiveScale() {
  const w = window.innerWidth || SCALE_DESIGN_W;
  const scale = Math.max(0.6, Math.min(1, w / SCALE_DESIGN_W));
  // innerWidth is NOT affected by root zoom, so this never feedback-loops. A small
  // tolerance (treat ≥0.985 as 1) absorbs scrollbar/sub-pixel variance so a window
  // at-or-near the default size never zooms.
  document.documentElement.style.zoom = scale >= 0.985 ? "" : String(scale);
}

/* ============================================================================
   READER — document-to-audio via OpenRouter TTS (owner 2026-06-20, rebuilt)
   Speaks ONLY through OpenRouter text-to-speech models (AI voices) — the system
   Speech-Synthesis path is gone. Keeps a LIBRARY of opened documents and resumes
...
function readerBuildPane(text) {
  ...
  // (≈ a 1000-page book) and tell the user — never truncate silently.
  ...
  if (plain.length > MAX_CHARS) {
    ...
    let cut = plain.lastIndexOf(" ", MAX_CHARS);
    if (cut < MAX_CHARS * 0.9) cut = MAX_CHARS; // no nearby space — hard cut
...
/* Render parsed document blocks (headings, paragraphs, tables) into the
   reading pane. Table blocks are rendered as HTML <table> elements with
   <thead>/<tbody>. All blocks also contribute their text to a combined
   plain-text stream built via readerBuildPane for TTS word highlighting. */
function readerBuildBlocks(blocks) {
  ...
  blocks.forEach(function (block) {
    ...
    if (type === "table") {
      ...
      if (headers.length) {
        tbl += '<thead><tr>';
        headers.forEach(function (h) { tbl += '<th scope="col">' + esc(String(h)) + '</th>'; });
        tbl += '</tr></thead>';
      }
      tbl += '<tbody>';
      rows.forEach(function (row) {
        ...
        (row || []).forEach(function (cell) {
          tbl += '<td>' + esc(String(cell != null ? cell : "")) + '</td>';
        });
        tbl += '</tr>';
      });
      tbl += '</tbody></table>';
      ...
      tbl += '</div>';
      tablesHtml += tbl;
      // Add flattened table text for TTS
      var flat = [];
      if (headers.length) flat.push(headers.join(", "));
      rows.forEach(function (row) { flat.push((row || []).join(", ")); });
      plainText += (plainText ? "\n\n" : "") + flat.join(". ");
    } else {
      plainText += (plainText ? "\n\n" : "") + text;
    }
  });
  // Build the plain-text pane with word highlighting
  readerBuildPane(plainText);
  // Insert table HTML at the end (after all plain-text paragraphs)
  if (tablesHtml) {
    pane.insertAdjacentHTML("beforeend", tablesHtml);
  }
  // Update plain for chunk building
  // (readerBuildPane already set READER.plain and READER.words)
}
...
function readerUpdateProgress() {
  ...
  const pct = n ? Math.round((100 * (READER.idx + 1)) / n) : 0;
  setText("#reader-progress", n ? `${pct}% • word ${READER.idx + 1} of ${n}` : "");
}
...
async function readerFetchChunk(ci) {
  ...
  try {
    ...
    READER.cache[ci] = obj;
    // Update synthesis progress in the UI.
    READER.synthDone++;
...
async function readerPlayChunk(ci, seekWord) {
  ...
  audio.onerror = () => {
    if (gen !== READER.gen || READER.state !== "playing") return; // superseded clip
    // A chunk that decodes-but-fails used to skip silently to the next with no
    // cap, so a doc that fails on EVERY chunk ran to "Finished." with no error.
    // Stop after 3 consecutive failures and tell the user instead.
    READER.errStreak = (READER.errStreak || 0) + 1;
    if (READER.errStreak >= 3) {
      READER.errStreak = 0;
      READER.state = "paused";
      readerSetPlayIcon();
      const msg = "Couldn't play the audio (3 parts failed in a row). Check the voice model and your OpenRouter key, then press play.";
      readerSetStatus(msg);
      toast(msg, "err", 5000);
      return;
    }
  ...
  try {
    ...
    readerSetStatus("Press play to start (the browser blocked autoplay).");
  }
...
function readerPlay() {
  ...
  if (
    ...
    READER.audio &&
    !READER.audio.ended &&
    READER.audio.currentTime > 0
  ) {
    READER.state = "playing";
    readerSetPlayIcon();
    READER.sessionStart = Date.now();      // start a new session segment
    READER.sessionStartPos = READER.idx;   // log from current word
    READER.audio.play().catch(() => {});
    return;
  }
  READER.state = "playing";
  readerSetPlayIcon();
...
function readerReload() {
  ...
  READER.fallbackUsed = false;
  READER._fallbackToasted = false;
  if (READER.state === "playing") readerPlayChunk(READER.chunkIdx, READER.idx);
}
...
function _readerLogSession() {
  ...
  const dur = READER.sessionStart ? (Date.now() - READER.sessionStart) / 1000.0 : 0;
  ...
  const completed = docLen > 0 && endPos >= docLen - 1;
...
async function readerRenderAllTab(useCache) {
  ...
  host.innerHTML = docs
    ...
    <div class="rl-item" role="listitem" data-open="${esc(d.id)}" tabindex="0" aria-label="${esc(d.title)} — ${d.format ? d.format.toUpperCase() : 'TXT'} document">
      ...
      <button class="btn btn-sm btn-gold rl-resume" data-open="${esc(d.id)}" aria-label="${d.percent > 0 ? 'Resume reading' : 'Read'} ${esc(d.title)}">${d.percent > 0 ? "Resume" : "Read"}</button>
  ...
  host.innerHTML =
    ...
    (docs.length ? docs.map(function (d) {
      return `<div class="rl-item" data-open="${esc(d.id)}">
        ...
        <button class="btn btn-sm btn-gold rl-resume" data-open="${esc(d.id)}">${d.percent > 0 ? "Resume" : "Read"}</button>
      </div>`;
...
function readerShowAddToCollection(docId, anchorEl) {
...
function readerShowPlayer(on) {
  ...
  $("#reader-player").hidden = !on;
  if (wide) {
    // Library sidebar stays visible beside the player pane.
    // Hide #reader-input when the player is shown so the grid
    // has exactly 2 children (library + player) in 2 columns.
    $("#reader-library").hidden = false;
    $("#reader-input").hidden = on;
  } else {
    $("#reader-library").hidden = on;
    $("#reader-input").hidden = on;
  }
}

async function readerOpenDoc(id) {
  READER.gen++; // abort any in-flight chunk from a previously-open document
  ...
  READER.audio = null;
  READER.state = "idle";
  READER.idx = Math.max(0, Math.min(doc.position || 0, READER.words.length - 1));
...
/* ============================================================================
   ...
   ========================================================================== */
const MEET = { recording: false, paused: false, recTimer: null, recStart: 0, openId: null, wired: false };
...
function meetingWire() {
  ...
  $("#meeting-star-detail")?.addEventListener("click", async () => {
    ...
    await call("meeting_star", MEET.openId, true);
    openMeeting(MEET.openId);
    toast("Starred", "ok", 1200);
  });
  $("#meeting-del-detail")?.addEventListener("click", async () => {
    ...
    if (!yes) return;
    await call("meeting_delete", MEET.openId);
    MEET.openId = null;
    renderMeetings();
...
async function renderMeetings() {
  ...
  const list = (await call("meeting_list")) || [];
  ...
  wrap.innerHTML = list.map((m) => {
    var modeBadge = "";
    if (m.processing_mode === "deep" && (m.decision_count > 0 || m.question_count > 0)) {
    ...
    return `
    <div class="card pad lift meeting-item" data-id="${esc(m.id)}" style="cursor:pointer">
      <div class="flex items-center justify-between gap8">
        <div style="min-width:0;flex:1">
          <div class="flex items-center gap8">
            ${m.starred ? '<span class="t-gold" style="width:13px;height:13px;display:inline-flex;flex:none">'+svg("star")+'</span>' : ''}
            <div class="fs13 fw6" style="${_ell}">${esc(m.title)}</div>
            ${modeBadge}
          </div>
          <div class="t-mute fs11" style="margin-top:2px">${esc(m.duration_display)} · ${m.speaker_count} speaker${m.speaker_count===1?"":"s"} · ${m.segment_count} segments${summarisedBadge}</div>
          ${m.preview ? `<div class="t-dim fs12" style="margin-top:6px;${_ell}">${esc(m.preview)}</div>` : ""}
        </div>
        <div class="flex items-center gap4" style="flex:none">
          <button class="btn btn-icon meeting-star" data-id="${esc(m.id)}" title="Star">${svg("star")}</button>
          <button class="btn btn-icon meeting-del" data-id="${esc(m.id)}" title="Delete">${svg("trash")}</button>
        </div>
      </div>
    </div>`;
  }).join("");
  wrap.querySelectorAll(".meeting-item").forEach((el) => {
    el.addEventListener("click", (e) => {
      if (e.target.closest(".meeting-star,.meeting-del")) return;
      openMeeting(el.dataset.id);
    });
  });
  wrap.querySelectorAll(".meeting-star").forEach((b) => b.addEventListener("click", async (e) => {
    e.stopPropagation(); await call("meeting_star", b.dataset.id, true); renderMeetings();
  }));
  wrap.querySelectorAll(".meeting-del").forEach((b) => b.addEventListener("click", async (e) => {
    e.stopPropagation();
    var mid = b.dataset.id;
    var m = (MOCK.meetings || []).find(function (x) { return x.id === mid; }) || {};
    var title = m.title || "this meeting";
    const yes = await confirmModal({ icon: "trash", title: "Delete " + esc(title) + "?", body: "The transcript and recording are permanently removed. This cannot be undone.", confirmText: "Delete" });
    if (!yes) return;
    await call("meeting_delete", mid); renderMeetings();
  }));
  paintIcons(wrap);
}
...
async function openMeeting(id) {
  ...
  tx.innerHTML = (m.segments || []).map(function (seg) {
  ...
  }).join("") || '<p class="help text-center">No transcript for this meeting.</p>';
...
async function meetingTogglePause() {
  ...
  if (MEET.paused) {
    // Resume
    await call("meeting_resume_recording");
    MEET.paused = false;
    ...
    MEET.paused = true;
    await call("meeting_pause_recording");
...
async function meetingImport() {
  ...
  if (r && r.ok) { toast("Audio imported + transcribed", "ok"); renderMeetings(); }
...
async function meetingExport() {
  ...
  var r = await call("meeting_export", MEET.openId, fmt);
  if (r && r.ok) {
    if (fmt === "clipboard" && r.content) {
      navigator.clipboard?.writeText(r.content).catch(function () {});
      toast("Copied to clipboard (" + fmt + ")", "ok", 2000);
    } else {
      toast("Exported as " + fmt.toUpperCase(), "ok", 2000);
    }
  } else {
...
async function initReader() {
  ...
  $("#reader-create-collection")?.addEventListener("click", async () => {
    ...
    // Add a dummy doc-to-collection doesn't make sense — just switch to collections
    // and let the user add docs. Creating a collection is really about naming it;
    // the actual membership is managed on the All tab.
    // But we need at least one doc to create a collection (collections are labels on docs).
    // For UX, we create an empty collection by ensuring the name is valid, and it
    // will appear once a doc is added to it. Actually, let's rethink — collections
    // only exist as labels on docs, so creating one without a doc is a no-op.
    // Instead, make this clear: switch to All tab so they can add docs.
    toast("Collection names are created when you add a document to them — use the + button on any document in the All tab.", "info", 4000);
    if (inp) inp.value = "";
  });
...
async function boot() {
  ...
  window.addEventListener("resize", debounce(function () {
    ...
    if (wideNow !== _rdrWide) {
      ...
      if (READER.docId && READER.inited) {
        ...
        var playerOn = READER.state !== "idle";
        readerShowPlayer(playerOn);
      }
    }
  }, 120));
  initReader();
  ...
  // Filter chips — toggleable content-type filters (owner v7)
  $$("#hist-filterchips .filterchip").forEach(
    (b) => (b.onclick = () => toggleFilter(b.dataset.filter)),
  );
  ...
  // The Big Shift onboarding: PC class + language choice cards (single-select).
  $$("#ob-hw .ob-choice").forEach(
    (b) =>
      (b.onclick = () => {
        $$("#ob-hw .ob-choice").forEach((x) => x.classList.remove("active"));
        b.classList.add("active");
        OB.tier = b.dataset.tier;
      }),
  );
  $("#ob-hw-auto")?.addEventListener("click", () => {
    $$("#ob-hw .ob-choice").forEach((x) => x.classList.remove("active"));
    OB.tier = "auto";
    obNext();
  });
...
function wireSettingsControls() {
  ...
  const SHARE_TEXT = [
    ...
    "pastes at your cursor. Your audio never leaves your machine (it",
    "transcribes on-device); an optional AI polish shapes emails, lists",
    "and prompts using your own free API key.",
    "",
    "→ Get it: https://github.com/YOUR-GITHUB/mumble  (placeholder link)",
  ].join("\n");
  const prev = $("#share-preview");
  if (prev) prev.textContent = SHARE_TEXT;
  $("#set-share-copy")?.addEventListener("click", () => {
    navigator.clipboard?.writeText(SHARE_TEXT).catch(() => {});
    flash($("#set-share-fb"), "Copied — paste it anywhere", "ok");
  });
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  (function () {
    ...
    if (defaultTheme === 'gold-black') {
      activeTheme = localStorage.getItem('mumble-app-theme') || defaultTheme;
    }
    ...
    function applyTheme(themeId) {
      ...
      localStorage.setItem('mumble-app-theme', themeId);
