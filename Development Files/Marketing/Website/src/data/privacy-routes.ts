...
export const privacyRoutes = [
  ...
  },
  {
    ...
    facts: {
      ...
      provider: 'The configured OpenRouter or OpenAI speech provider. A compatible-model recovery may occur only within the same provider.',
      ...
      failureBoundary: 'A missing/invalid key, device-only policy, unavailable provider/model, exhausted credits, network failure, or synthesis error stops speech. There is no local speech fallback and no switch to another provider.',
    },
  },
  {
    panelId: 'mumble-find',
